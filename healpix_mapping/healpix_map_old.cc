...
int main(int argc, char **argv) {
  ...
  string filter_name;
  int run=12;
  string temp = "/data/anita/btdailey/10sample_outputs/%s/output%d_0_0.root";
  char *rootfile;
  filter_name="geom";
   rootfile = Form(temp.c_str(),filter_name.c_str(),run);
     ...
     for(int extra=13;extra<261;extra+=1){
    
       sprintf(rootfile,"/data/anita/btdailey/10sample_outputs/%s/output%d_0_0.root",filter_name.c_str(),extra);
  ...
  //then use ang2pix  to get pix, add event to Base[pixel]
     ...
     double x_map;
      ...
      vector< vector <double> > healpix_bin_weights (n_pix_int, vector<double>(1));
	 ...
	 //cout<<"anita is at "<<anita_pointx<<" "<<anita_pointy<<"\n";
	   ...
	   if(errorpoints_return >2){//use only points that intersect the earth the correct number of times. 
	     ...
	     for(int i=0;i<100;i++){
	       ...
	       //////////////get boundaries in payload coordinates and then cone coordinates/////////////
      ...
      cout<<"number of events filled into bins is "<<base_ctr<<"\n";
      cout<<"\n errorellipse is "<<errorellipse_ctr<<"\n";
      cout<<"events in more than 1 bin: "<<doublebinctr<<" form cone coordinates it is "<<doublebinctr_cone<<"\n";
	...
	//point_boundaries[j]->Draw("same");
      ...
      cutLine->SetLineColor(kRed);
       ...
       //TH2F *hRotatedCutWhole = new TH2F("rotatedcut whole",";Peak Cross Corr Val;Corrected Peak Hilbert Value/1E6 (mV m);",100,0,1,100,0,100);
      for(int n=1;n<numberevents;n++){
	//	cout<<"peakVal, peakHilbert is "<<peakVal_vector[n]<<" "<<peakHilbert_vector[n]<<"\n";
	hRotatedCut->Fill(peakVal_vector[bin_number][n], peakHilbert_vector[bin_number][n],healpix_bin_weights[bin_number][n]);
	//hRotatedCutWhole->Fill(peakVal_vector[bin_number][n], peakHilbert_vector[bin_number][n]);
      }
      TCanvas *c3 = new TCanvas("c3","c3",800,800);
      hRotatedCut->Draw("zcol");
      cutLine->Draw("same");
      sprintf(printer,"rotatedCut_%i_%i.png",bin_number,whole_flag);
      c3->Print(printer);
      ...
      for(int sloper=20;sloper<30;sloper++){
	slope = -1*sloper;
	cout<<" slope is "<<slope<<"\n";
	bad_fit_ctr=0;
	
	TH1D *hChiSquare = new TH1D("ChiSquare",";ChiSquare/NDF;Number of Events",100,0.,200.);
	TH1D *hpVal = new TH1D("pVal",";p Val;Number of Events",101,0.,1.01);
	...
	for(int m=0;m<n_pix_int;m++){
	  ...
	  if(numevents[m]>0) {
	    ...
	    TH1D *hdiffplotWhole = new TH1D("diffplotWhole",";y_intercept;Number of Events Cut;",1000,0.,1000.);
	    TH1D *hdiffplotTotal = new TH1D("diffplotTotal",";y_intercept;Number of Events Cut;",1000,0.,1000.);
	    hdiffplot->Sumw2();
	    TH2F *hRotatedCut = new TH2F("rotatedcut",";Peak Cross Corr Val;Corrected Peak Hilbert Value/1E6 (mV m);",100,0,1,100,0,100);
	    max_weight=0;
		...
		if(peakHilbert_vector[m][n] < slope*peakVal_vector[m][n]+y_int && peakHilbert_vector[m][n] > slope*peakVal_vector[m][n]+y_int-1 ){ 
		  //if(m==3066) cout<<"cut! \n";
	    ...
	    TH1D *hpseudoVal = new TH1D("pseudoVal","likelihood value;Counts",1000,0,100);
	    TH2D *hpvalDist = new TH2D("pvalDist",";Log(Number of events);PValue;",10,0,3,10,0,1);
	    ...
	    haxes_diff = new TH2F("axes_diff",";y_int;Number of Events Cut",200,0,fit_end+10,200,.1*min_weight,num_cut_max*2);
	    if(fit_end >fit_start+1 && numberevents >1){
	      cout<<"doing fit \n";
		...
		if(x_val >= fit_start && x_val <= fit_end){
		  ...
		  */
	      ...
	      double y_rndm;
	      double x_rndm;
	      double y_min = exp(yint+slope*fit_start);
	      double y_max = exp(yint+slope*fit_end);
		...
		TH1D *hpseudo = new TH1D("psuedo",";xval;yval",1000,0.,1000.);
					
		hpseudo->Sumw2();
		//cout<<"num_events_pseudo is "<<num_events_pseudo<<"\n";
		for(int j=0;j<num_events_whole;j++){
		  y_rndm = gRandom->Rndm();
		  x_rndm = (y_max-y_min)*y_rndm + y_min;
		  x_rndm = log(x_rndm);
		  x_rndm = (x_rndm - yint)/slope;
		  ...
		  if(rndm < (1-percentwhole) ) {
		    if(max_weight <1){
		    weight = (max_weight/num_events_total + max_weight)*gRandom->Rndm();
		    }
		    else {
		      weight = gRandom->Rndm();
		    }
		...
		cout<<"m is "<<m<<" sloper is "<<sloper<<" numevents is "<<numberevents<<" pval is "<<pval<<"\n";
	      ...
	      TH2D *haxes_like = new TH2D("Axes_like",";Likelihood;Number of Counts",10,0,max_likelihood,10,0,1);
	      TLine *line = new TLine(likelihood_val,0,likelihood_val,1);
	      line->SetLineColor(kRed);
	      haxes_like->Draw();
	      line->Draw("same");
	      hpseudoVal->Draw("same");
	      sprintf(printer,"likelihood_%i_%i.png",m,sloper);
	    ...
	    hpvalDist->Draw("zcol");
	    sprintf(printer,"pvalDist_%i.png",sloper);
	    ...
	    */
	    ...
	    //if(sloper==22){ 
	      ...
	      //fit->SetStats(0);
	      //c4->Update();
	      sprintf(printer,"diffplot_%i_%i_%i.png",m,sloper,whole_flag);
	      c4->Print(printer);
	      delete c4;
	      delete haxes_diff;
	    }
	    else if (numberevents >1){

 ...
 double xholder_temp[100]={0.};
  ...
  double hilbertctr=0.;
   ...
   for(int i=0;i<15;i++){
     ...
     hdenom_Brian[i]= new TH1D(printer,";SNR;Efficiency",20,0,200);

     sprintf(printer,"efficiency_Brian_%i",i);
     hefficiency_Brian[i]= new TH1D(printer,";SNR;Efficiency",20,0,200);
	 ...
	 for(int k=0;k<4;k++){
	   ...
	   if(area_pix[k]>0){
		...
		//cout<<"snrcoherent is "<<snrcoherent<<"\n";
	       hBrian[bin_index]->Fill(snrcoherent,areas[k]);
	       //cout<<"Added \n";
   ...
   c0->Print("efficiency_background.png");

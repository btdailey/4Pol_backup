...
analysis_info_4pol *pol4_PtrA = 0;
...
int main(int argc, char **argv) {
 ...
 double xholder_temp[100]={0.};
  ...
  ////ABBY rootfile
   ...
   temp = "/data/anita/btdailey/final_filter/simulated/Abby/output%d_0.root";
  
  
   rootfile = Form(temp.c_str(),run);
  
 
  TChain *pol4_TreeA = new TChain("analysis_info_4pol");
  TChain *Pointed_TreeA =  new TChain("tdataTracing");
  pol4_TreeA->Add(rootfile);
  Pointed_TreeA->Add(rootfile);
  //extras
  ...
  int neventsA = pol4_TreeA->GetEntries();
  
  pol4_TreeA->SetBranchAddress("pol4_Ptr",&pol4_PtrA);
  ...
  double hilbertctr=0.;
   ...
   for(int i=0;i<num_bins;i++){
     ...
     sprintf(printer,"denom_Brian_%i",i);
     hdenom_Brian[i]= new TH1D(printer,";SNR;Efficiency",10,0,200);

     sprintf(printer,"efficiency_Brian_%i",i);
     hefficiency_Brian[i]= new TH1D(printer,";SNR;Efficiency",10,0,200);
    
   }
   hefficiency_Brian[0]->SetLineColor(kRed);
   hefficiency_Brian[1]->SetLineColor(kBlue);
   hefficiency_Brian[2]->SetLineColor(kGreen);
   hefficiency_Brian[3]->SetLineColor(kYellow);
   hefficiency_Brian[4]->SetLineColor(kMagenta);
   hefficiency_Brian[5]->SetLineColor(kCyan);
   hefficiency_Brian[6]->SetLineColor(kOrange);
   ...
   hefficiency_Brian[2]->SetMarkerColor(kGreen);
   hefficiency_Brian[3]->SetMarkerColor(kYellow);
   hefficiency_Brian[4]->SetMarkerColor(kMagenta);
   hefficiency_Brian[5]->SetMarkerColor(kCyan);
   hefficiency_Brian[6]->SetMarkerColor(kOrange);
   ...
   hefficiency_Brian[2]->SetMarkerStyle(22);
   hefficiency_Brian[3]->SetMarkerStyle(23);
   hefficiency_Brian[4]->SetMarkerStyle(29);
   hefficiency_Brian[5]->SetMarkerStyle(33);
   hefficiency_Brian[6]->SetMarkerStyle(34);
   hefficiency_Brian[7]->SetMarkerStyle(20);
   hefficiency_Brian[8]->SetMarkerStyle(21);
   hefficiency_Brian[9]->SetMarkerStyle(22);
   hefficiency_Brian[10]->SetMarkerStyle(23);
   hefficiency_Brian[11]->SetMarkerStyle(29);
   ...
   for(int m=0;m<nevents0;m+=1){
	...
	pol4_TreeA->GetEvent(m);
	Pointed_TreeA->GetEvent(m);
       


       eventNumber_cut = pol4_PtrA->eventNumber;
	 ...
	 if(eventNumber_cut==eventNumber_icemc){
       ...
       ////////ABBY!! //////////
       peakHilbertCoherent=pol4_PtrA->peakHilbertCoherent[0];
       peakVal=pol4_PtrA->peakVal[0];
       ratioFirstToSecondPeak=pol4_PtrA->ratioFirstToSecondPeak[0];
       thetaMap=pol4_PtrA->thetaMap[0];
       phiMap = pol4_PtrA->phiMap[0];
       snrcoherent = pol4_PtrA->SNR_coherent[0];
       polFractionCoherent=pol4_PtrA->polFractionCoherent[0];
       anitaLat = pol4_PtrA->anitaLat;
       anitaLon = pol4_PtrA->anitaLon;
       anitaAlt = pol4_PtrA->anitaAlt;
       anitaHeading = pol4_PtrA->heading; 

       hwTriggerFlag=pol4_PtrA->hwTriggerFlag[0];
       snrPeak = pol4_PtrA->snrPeakAfterFilter[0];
       
       varnerFlag = pol4_PtrA->varnerFlag[0];
       didIFilter = pol4_PtrA->didIFilter[0];
       eventTracedFlag2 = pol4_PtrA->eventTracedFlag[0];
       varnerFlag2 = pol4_PtrA->varnerFlag2[0];
       ...
       badNoiseFlag = pol4_Ptr->noiseFlag[0];
        ...
        //PAYLOAD BLASTS
       if(pol4_PtrA->payloadblastflag ==1){
	 //payloadBlastctr+=weight;
	 continue;
       }

       hdenom->Fill(snrcoherent,weight);
	...
	*/
	...
	if(peakHilbertCoherent>=-350*peakVal+57.14){
	     //cout<<"passed Abby!, event is "<<eventNumber_cut<<" weight is "<<weight<<"\n";
	     hAbby->Fill(snrcoherent,weight);
	} 
   ...
   n_icemc=0;
	...
	//cout<<"pixel_num, y_int is "<<pixel_num<<" "<<y_int<<"\n";
	 ...
	 if(errorpoints_return <4) errorellipse_ctr++;//keep track of number of events that only intersect earth 2 times
	   ...
	   if(errorpoints_return >2){//use only points that intersect the earth the correct number of times. 
	     ...
	     for(int k=0;k<100;k++){
	       ...
	       //if(k==0) cout<<" ellipse 0 point is "<<90-Lats[k]<<" "<<180+Lons[k]<<"\n";
	       //cout<<"k is "<<k<<" lat lon for error is "<<90-Lats[k]<<" "<<180+Lons[k]<<"\n";
	 ...
	 if(areas[k]>0){
	   for(int i =0;i<healpix_bin_vector.size();i++){
	     if(healpix_bin_vector[i] == area_pix[k]){
	       
	       bin_index = i;
	     }
	   }
	   //cout<<"bin_idex is "<<bin_index<<"\n";
	   if(bin_index <100){
	   hdenom_Brian[bin_index]->Fill(snrcoherent,weight*areas[k]);
	   }
	 }
       }//k
	...
	*/
	...
	datactr2+=weight;
	//if(errorpoints_return<2) continue;
	 for(int k=0;k<4;k++){
	   
	   if(area_pix[k]>0){
	     //cout<<"pixel_num (y_int), area_pix, area is "<<pixel_num_event<<"("<<y_int<<") "<<area_pix[k]<<" "<<areas[k]<<" elevation is "<<thetaMap<<"\n";
	     y_int=100000;
	     for(int i =0;i<healpix_bin_vector.size();i++){
	       if(healpix_bin_vector[i] == area_pix[k]){
		 y_int = cut_val_vector[i];
		...
		//cout<<"snrcoherent, peakVal, y_int, area_pix are "<<snrcoherent<<" "<<peakVal<<" "<<y_int<<" "<<area_pix[k]<<"\n";
		//cout<<"adding "<<areas[k]*weight<<"\n";
		//cout<<"snrcoherent is "<<snrcoherent<<"\n";
	       hBrian[bin_index]->Fill(snrcoherent,weight*areas[k]);
	       //cout<<"Added \n";
   ...
   for(int i=1;i<11;i++){
     ...
     numA+=hAbby->GetBinContent(i+1);
     denomA += hdenom->GetBinContent(i+1);

     numA+=hAbby->GetBinContent(i+2);
     denomA += hdenom->GetBinContent(i+2);
     cout<<"i numA, denomA are "<<i+1<<" "<<numA<<" "<<denomA<<"\n";
     if(denomA < 1E-5) denomA=1;
     hefficiency_Abby->SetBinContent(i+1,numA/denomA);

     for(int j=0;j<15;j++){
       numB = hBrian[j]->GetBinContent(i);
       denomB = hdenom_Brian[j]->GetBinContent(i);

       numB += hBrian[j]->GetBinContent(i+1);
       denomB += hdenom_Brian[j]->GetBinContent(i+1);

       numB += hBrian[j]->GetBinContent(i+2);
       denomB += hdenom_Brian[j]->GetBinContent(i+2);
       if(denomB < 1E-5) denomB=1;
       hefficiency_Brian[j]->SetBinContent(i+1,numB/denomB);
       cout<<"i numB, denomB are "<<i+1<<" "<<numB<<" "<<denomB<<"\n";
     }
   
     }//i
   */
   TCanvas *c0 = new TCanvas("c0","c0",880,800);
   TH2F  *haxes = new TH2F("haxes",";SNR;Efficiency",10,0,200,10,0,1.2);
   haxes->Draw();
   hefficiency_Abby->Draw("same l P");
   TLegend leg (.3,.85,.6,1,"l");
   leg.SetFillColor(0);
   ...
   for(int j=0;j<10;j++){
    ...
    for(int j=10;j<20;j++){
     ...
     hefficiency_Brian[j]->Draw("same l P");
     sprintf(printer,"HealPix Bin %i",healpix_bin_vector[j]);
     leg2.AddEntry(hefficiency_Brian[j],printer);
   }

   leg.Draw("same");
   leg2.Draw("same");

   c0->Print("efficiency.png");

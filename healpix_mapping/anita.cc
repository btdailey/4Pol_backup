...
Anita::Anita() {
    ...
    coherent_datafile = new TFile("outputs/coherent_sum_data.root","RECREATE");
...
void Anita::Initialize(Settings *settings1,ofstream &foutput,int inu) 
{
    ...
    tsignals->Branch("total_diodeinput_1_inanita",&total_diodeinput_1_inanita,"total_diodeinput_1_inanita[5][512]"); // this is the waveform that is input to the tunnel diode in the first (LCP or vertical) polarization
    tsignals->Branch("total_diodeinput_2_inanita",&total_diodeinput_2_inanita,"total_diodeinput_2_inanita[5][512]"); // this is the waveform that is input to the tunnel diode in the first (RCP or horizontal) polarization
...
void Anita::AntennaGain(Settings *settings1,double hitangle_e,double hitangle_h,double e_component,double h_component,int k,double &vsignalarray_e,double &vsignalarray_h) {
	...
	vsignalarray_e=vsignalarray_e*0.5* // 0.5 is for voltage dividing
	sqrt(vvGaintoHeight[k]*vvGaintoHeight[k]*e_component*e_component*relativegains[0]+
	     hvGaintoHeight[k]*hvGaintoHeight[k]*h_component*h_component*relativegains[1]);
	...
	sqrt(hhGaintoHeight[k]*hhGaintoHeight[k]*h_component*h_component*relativegains[2]+
	     vhGaintoHeight[k]*vhGaintoHeight[k]*e_component*e_component*relativegains[3]);
...
void Anita::Set_gain_angle(Settings *settings1,double nmedium_receiver) {
	...
	//now a linear interpolation for the frequancy
...
void Anita::fill_coherent_waveform_sum_tree(unsigned event_number, Settings* settings1, double rms_noise, double actual_rms, unsigned window_start, unsigned window_end, double deg_theta, double deg_phi, double timesteps[], vector <vector <vector <double> > >& whole_wfms, vector <vector <vector <double> > >& wfms, vector <vector <double > >& aligned_wfms, vector <double>& summed_wfm, vector <double>& power_of_summed_wfm, double power){
    ...
    for (int i = 0; i < HALFNFOUR; i++){
	cwst_timesteps[i] = timesteps[i];
    }
    
    // Reassign the nested vector associated with the whole_wfms branch
    for (unsigned int i = 0; i < whole_wfms.size(); i++){
	for (unsigned int j = 0; j < whole_wfms[i].size(); j++){
	    *(cwst_whole_wfms[whole_wfms.size() * i + j]) = whole_wfms[i][j];
	}
    }
    
    // Reassign the nested vector associated with the wfms branch
    for (unsigned int i = 0; i < wfms.size(); i++){
	for (unsigned int j = 0; j < wfms[i].size(); j++){
	    *(cwst_wfms[wfms.size() * i + j]) = wfms[i][j];
	}
    }
    
    // Reassign the nested vector associated with the aligned_wfms branch
    for (unsigned int i = 0; i < aligned_wfms.size(); i++){
	*(cwst_aligned_wfms[i]) = aligned_wfms[i];
    }
...
void Anita::GetPayload(Settings* settings1, Balloon* bn1){
	...
	PHI_EACHLAYER[3][0] = -67.997 * RADDEG - gps_offset_anita2;
	PHI_EACHLAYER[3][1] = -22.948 * RADDEG - gps_offset_anita2;
	PHI_EACHLAYER[3][2] = 22.382 * RADDEG - gps_offset_anita2;
	PHI_EACHLAYER[3][3] = 67.583 * RADDEG - gps_offset_anita2;
	PHI_EACHLAYER[3][4] = 112.844 * RADDEG - gps_offset_anita2;
	PHI_EACHLAYER[3][5] = 157.761 * RADDEG - gps_offset_anita2;
	PHI_EACHLAYER[3][6] = -157.896 * RADDEG - gps_offset_anita2;
	PHI_EACHLAYER[3][7] = -112.791 * RADDEG - gps_offset_anita2;
	...
	PHI_OFFSET[1]=-2.*PI/(double)NRX_PHI[0]/2.;
	PHI_OFFSET[2]=-2.*PI/(double)NRX_PHI[0]/2.;
	PHI_OFFSET[3]=-2.*PI/(double)NRX_PHI[0]/2.;

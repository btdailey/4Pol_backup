...
int main(int argc, char **argv) {
 ...
 int run=1;
  ...
  temp = "HealPix_sim_partial_1215.root";
	...
	datactr2+=weight;
	 ...
	 for(int k=0;k<4;k++){
	   ...
	   if(area_pix[k]>0){
	     ...
	     if(snrcoherent >= -38*peakVal + y_int) datactr+=(weight*areas[k]);
	   }
	 }
	cout<<"event "<<eventNumber_cut<<" pixel_num is "<<pixel_num_event<<" passed! \n";
   }//m

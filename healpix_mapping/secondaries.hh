...
string flavors[3]; // the gps path of the anita-lite flight  
  ...
  double GetTauWeight(Primaries *primary1, Settings *settings1,IceModel*antarctica1, double pnu, int nu_nubar, 
				 int currentint, double& Etau_final, const Position posnu, const Position earth_in,
				 int& crust_entered, // 1 or 0
				 int& mantle_entered, // 1 or 0
				 int& core_entered, double *prob_at_z_vector, double *distance_vector, double *prob_at_z_vector1,
		                 double *distance_vector1, double *Energy_vector, double *Energy_vector1,
				 double *nusurvarray, double *dEtauidEtaufarray,
  ...
  double dTauSurvdx(double& probSurv1,double ptau_final,double step);
  double TauEnergyInitial(double ptau_final,double Distance,double z_distance,double density);
  double TauEnergyFinal(double ptaui,double step,double density);
  double FindBin(double ptau_final);
  double GetNuTauWeight(Primaries *primary1, Settings *settings1,IceModel*antarctica1,double pnu,double ptau_final,
			Position earth_in,Position posnu,Position r_enterice, Position nuexitice,
			double& weight1_tmp,int NNU,int nu_nubar, int currentint);
}; //class Secondaries

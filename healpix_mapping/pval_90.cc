...
int main(int argc, char **argv) {
  ...
  vector< vector <int> > eventNumber_vector (n_pix_int+1, vector<int>(1));
   ifstream myfile;
  myfile.open("Diff_fit_values_test.txt");
  ...
  // string temp = "HealPix_partial_0207.root";//HPol
   string temp = "HealPix_partial_0301_90sample.root";
  char *rootfile;
  ...
  //READ IN FIT VALUES
  vector<int> healpix_bin_number;
  vector<double> slope_vec;
  vector<double> y_int_vec;
  vector<double> fit_start_vec;
  vector<double> fit_end_vec;

  string line;
  while (getline(myfile,line)){
    istringstream ss(line);
    int number;
    double slope_fit_temp, y_int_temp, slope_start_temp,slope_end_temp;
    ss >> number >> slope_fit_temp >> y_int_temp >> slope_start_temp >> slope_end_temp;
    //cout<<number<<" "<<slope_fit_temp<<" "<<y_int_temp<<" "<<slope_start_temp<<" "<<slope_end_temp<<"\n";
    healpix_bin_number.push_back(number);
    slope_vec.push_back(slope_fit_temp);
    y_int_vec.push_back(y_int_temp);
    fit_start_vec.push_back(slope_start_temp);
    fit_end_vec.push_back(slope_end_temp);
  ...
  max_base = 1.1*max_base;
	...
	//for(int m=3032;m<3033;m++){
	  ...
	  //TH1D *hdiffplot = new TH1D("diffplot",";y_intercept;Number of Events Cut;",100,0.,100.);
		...
		for(int mm=0;mm<healpix_bin_number.size();mm++){
		  if(healpix_bin_number[mm]==m){
		    slope_fit = slope_vec[mm];
		    yint = y_int_vec[mm];
		    fit_start = fit_start_vec[mm];
		    fit_end = fit_end_vec[mm];
		    break;
		    
		  }

		}
		if(slope_fit==0.) continue;
		cout<<"slope_fit, y_int, fit_start, end are "<<slope_fit<<" "<<yint<<" "<<fit_start<<" "<<fit_end<<"\n";
	  ...
	  if(numberevents ==1) continue;
	    ...
	    TH1D *hdiffplot = new TH1D("diffplot",";y_intercept;Number of Events Cut;",num_bins_plots,0.,100000.);
	    TH1D *hdiffplotWhole = new TH1D("diffplotWhole",";y_intercept;Number of Events Cut;",num_bins_plots,0.,100000.);
	    TH1D *hdiffplotTotal = new TH1D("diffplotTotal",";y_intercept;Number of Events Cut;",num_bins_plots,0.,100000.);
		...
		if(peakVal_vector[m][n] < 0.075){
	     ...
	     for(double y_int=0;y_int<100000;y_int+=1){
	       num_cut=0.;
	       num_cut_int=0;
		 ...
		 if(cut_flag==1){ 
		   if(y_int > y_int_max) y_int_max = y_int;
		   //if(m==3032) cout<<"y_int, weight is "<<y_int<<" "<<healpix_bin_weights[m][n]<<"\n";
		 ...
		 y_int = 1E5;
		 fullcut_flag=1;
		 // cout<<"num_cut_old, fit_events are "<<num_cut_old<<" "<<fit_events<<"\n";
	     ...
	     cout<<"fit_events is "<<fit_events<<"\n";
	     TH2F *haxes_diff_0 = new TH2F("axes_diff",";y_int;Number of Events Cut",200,0,y_int_max+5,200,0.1*min_cut,num_cut_max*2);
	       TCanvas *c4_0 = new TCanvas("c4_0","c4_0",800,800);
		c4_0->SetLogy();
		
		haxes_diff_0->Draw();
		hdiffplot->Draw("sames");
		sprintf(printer,"diffplot_%i_90sample.png",m,sloper);
		c4_0->Print(printer);
	    ...
	    TH1D *hpseudoVal = new TH1D("pseudoVal","likelihood value;Counts",num_bins_plots*10,0,100000);
	    
	    double likelihood_val;
	    double likelihood_val_pseudo;
	    double edm, errdef;
	    int nvpar,nparx;
	    int num_events_plot = hdiffplot->GetEntries();
	    ...
	    haxes_diff = new TH2F("axes_diff",";y_int;Number of Events Cut",200,0,y_int_max+5,200,0.1*min_cut,num_cut_max*2);
		...
		cout<<"m is "<<m<<" cut_val_bin is "<<cut_val_bin<<"\n";
	      }
	    }

	    double x2 = -cut_val_bin/slope;
	    double y2=0.;
	    if(x2 >1) {
	      x2=1;
	      y2 = slope+cut_val_bin;
	    }

	     TLine *line2 = new TLine(0,cut_val_bin,x2,y2);
	    ...
	    //cutLine->Draw("same");
	    sprintf(printer,"rotatedCut_%i_90sample.png",m);
	    c3->Print(printer);
		...
		*/
	      ...
	      cout<<"y_min, max are "<<y_min<<" "<<y_max<<"\n";
	      double rndm;
	      double weight;
	      ...
	      for(int exp_num=0;exp_num<10;exp_num++){
		//num_events_pseudo= gRandom->Poisson(num_events_total);
		//cout<<"num_events_pseudo, num_events_total is "<<num_events_pseudo<<" "<<num_events_total<<"\n";
		TH1D *hpseudo = new TH1D("psuedo",";xval;yval",num_bins_plots,0.,100000.);
		...
		TF1 *myexpo = new TF1("myexpo","9*exp([0]+[1]*x)");
		...
		//	cout<<"slope_fit = "<< hpseudo->GetParameter(1)<<" y_int is "<<hpseudo->GetParameter(0)<<"\n";
		...
		sprintf(printer,"likelihood_%i_%i_90sample.png",m,sloper);
		...
		delete hRotatedCut;
	      ...
	      TF1 *myexpo = new TF1("myexpo","9*exp([0]+[1]*x)",fit_start,fit_end);
		myexpo->FixParameter(0,yint);
		myexpo->FixParameter(1,slope_fit);
		myexpo->SetLineColor(kRed);

	      TF1 *myexpo_old = new TF1("myexpo_old","exp([0]+[1]*x)",fit_start,fit_end);
	        myexpo_old->FixParameter(0,yint);
		myexpo_old->FixParameter(1,slope_fit);
		myexpo_old->SetLineColor(kBlue);
		...
		//TLatex *myt2 = new TLatex(.8,.5,printer);
	      ...
	      c4->Modified();
	      //c4->Update();
		sprintf(printer,"diffplot_%i_%i_%i_90sample.png",m,sloper,whole_flag);
		c4->Print(printer);
		delete c4;
		delete pt;
		delete haxes_diff;
	 ...
	 TCanvas *cpvalDist = new TCanvas("cpvalDist","cpvalDist",880,800);
       hpvalDist->Draw();
       sprintf(printer,"pvalDist_%i_90sample.png",sloper);

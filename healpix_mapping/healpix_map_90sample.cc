...
int main(int argc, char **argv) {
  ...
  //then use ang2pix  to get pix, add event to Base[pixel]
	...
	Lons_boundary1.push_back(Lons_boundary1_tmp);
      ...
      if(polarization==0){
	sprintf(filename,"HealPix_partial_0301_90sample.root");
      }
      if(polarization==1){
	sprintf(filename,"HealPix_partial_0301_90sample_HPol.root");
      }
...
*/
	...
	*/
	...
	lon = lon+180;//conversion
	 ...
	 //ONLY LOOKING FOR NEIGHBOR EVENTS TO PASSING EVENT!!!!!!!!!
	 /*if(pixel_num !=3051) continue;

	 cout<<"Event "<<pol4_Ptr->eventNumber<<" landed in bin "<<pixel_num<<". Lat and Lon are "<<lat<<" "<<lon<<"\n";
	 continue;
	 */
	...
	//if(BASE_allcuts[m] > max_base) max_base = BASE_allcuts[m];
      ...
      TH2D *hhealpix_map_used = new TH2D("healpix_map_used",";x(km);y(km);Number of Events",numbin_healpixmap,min_healpixmap,max_healpixmap,numbin_healpixmap,min_healpixmap-300,max_healpixmap-300);
	...
	hhealpix_map->Fill(x_map,y_map,BASE_allcuts[pixel_num]);
	  // cout<<"bin is "<<bin<<"\n";
	  //	if(pixel_num==3008 || pixel_num==3010 || pixel_num==3012 || pixel_num==3028 || pixel_num==3029 || pixel_num==3030 || pixel_num==3031 || pixel_num==3032 || pixel_num==3033 ||pixel_num==3037 || pixel_num==3045 ||pixel_num==3046 || pixel_num==3048 || pixel_num==3051 ||pixel_num==3052|| pixel_num==3053 || pixel_num==3057 || pixel_num==3061 || pixel_num==3062 || pixel_num==3063 || pixel_num==3066 || pixel_num==3069){
	if( pixel_num==3010 || pixel_num==3028 || pixel_num==3029 || pixel_num==3030 || pixel_num==3032 || pixel_num==3033 || pixel_num==3045 ||pixel_num==3046 || pixel_num==3048 || pixel_num==3051 || pixel_num==3053 || pixel_num==3057 || pixel_num==3061 || pixel_num==3062 || pixel_num==3063 || pixel_num==3066 || pixel_num==3069){
	  hhealpix_map_used->Fill(x_map,y_map,BASE_allcuts[pixel_num]);
		}
	  //  hhealpix_map->Fill(x_map,y_map);
	  //hhealpix_map->SetBinContent(bin,BASE_allcuts[pixel_num]);
	  
	  //cout<<"x_map,y_map,pixel,BASE_allcuts is "<<x_map<<" "<<y_map<<" "<<pixel_num<<" "<<BASE_allcuts[pixel_num]<<"\n";
	  //  }//pixel
	}
       }
      ...
      */
      ...
      if(polarization==0)sprintf(printer, "90sample_healpix_formed.root");
      if(polarization==1)sprintf(printer, "90sample_healpix_formed_HPol.root");
			    
      hhealpix_map->SaveAs(printer);

      if(polarization==0)sprintf(printer, "90sample_healpix_formed_used.root");
      if(polarization==1)sprintf(printer, "90sample_healpix_formed_HPol_used.root");
			    
      hhealpix_map_used->SaveAs(printer);
	...
	//point_boundaries[j]->Draw("same");
      ...
      sprintf(printer,"healpix_map_0501_90sample_%i.png",polarization);
      c2->Print(printer);
      sprintf(printer,"healpix_map_0501_90sample_%i.eps",polarization);
      c2->Print(printer);

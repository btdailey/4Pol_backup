...
int main(int argc, char **argv) {
  ...
  TMarker *error_ellipse_points[100000];
  ...
  ifstream pval_info;
  pval_info.open("Pval_info_626.txt");
  string line;
  ...
  vector<double> pval;
  //double v5;
  vector<double> fit_events;
  vector<double> fit_bins;
  while(getline(pval_info,line)){

    istringstream ss(line);
    int number;
    double v1t,v2t,v3t,v4t,v5t;
    double pval_tmp=-1.;
    double fit_events_tmp=-1.;
    ss >> number >>v1t>>v2t>>v3t>>v4t>>pval_tmp>>v5t>>fit_events_tmp;
    cout<<"number, pval, fit is "<<number<<" "<<pval_tmp<<" "<<fit_events_tmp<<"\n";
    healpixbin_cuts_vector.push_back(number);
    pval.push_back(pval_tmp);
    fit_events.push_back(fit_events_tmp);
    //cout<<"v4t,v3t, subtract is "<<v4t<<" "<<v3t<<" "<<v4t-v3t<<"\n";
    fit_bins.push_back(v5t);
  }
  ...
  string temp;
  if(polarization==0){
    temp= "/data/anita/btdailey/passingCuts/90sample_partial_passedcuts_0301.root";
  }
   if(polarization==1){
    temp= "/data/anita/btdailey/passingCuts/90sample_partial_passedcuts_0301_HPol.root";
  }
  //= "/data/anita/btdailey/passingCuts/90sample_partial_passedcuts_0301.root";
  char *rootfile;
  rootfile = Form(temp.c_str(),filter_name.c_str());
    
  TChain *pol4_Tree = new TChain("analysis_info_4pol");
  TChain *extraTree = new TChain("extraTree");
  ...
  //then use ang2pix  to get pix, add event to Base[pixel]
	...
	for(double n = 0;n<2*PI;n+=step_size){
	  offset = (int)(2*n/PI);
	  boundary_val = 1-pow(k,2.)/(3*pow(n_side,2.))*pow(PI/(2*(n-offset*PI/2)),2.);//equation of boundary
	  boundary_val1 = 1-pow(k,2.)/(3*pow(n_side,2.))*pow(PI/(2*(n-offset*PI/2)-PI),2.);//equation of boundary
	  ...
	  if(boundary_val <2./3.){
	    ...
	    SphericaltoCart(n,boundary_val,x_tmp,y_tmp);;//convert x/y to theta phi
	    
	    PhiTheta2LatLon(n,boundary_val,Lat_temp,Lon_temp);//convert to lat and lon
	    Lats_boundary_tmp.push_back(90-Lat_temp);//conversion of coordinates
	    Lons_boundary_tmp.push_back(Lon_temp-180);//conversion of coordinates
	  ...
	  if(boundary_val1 <2./3.){
	    ...
	    SphericaltoCart(n,boundary_val1,x_tmp,y_tmp);//convert x/y to theta phi
	    
	    
	    PhiTheta2LatLon(n,boundary_val1,Lat_temp,Lon_temp);//convert to lat and lon
	    Lats_boundary1_tmp.push_back(90-Lat_temp);//conversion of coordinates
	    Lons_boundary1_tmp.push_back(Lon_temp-180);//conversion of coordinates
	...
	//store Lats and Lons of boundaries
	...
	Lons_boundary1.push_back(Lons_boundary1_tmp);
      ...
      //int m_array[24]={9685,443564,690662,695405,754773,756166,756403,756673,757727,757734,829856,854766,1033787,1321750,1323535,1323624,1323647,1323668,1323703,1519687,1521383,1521464,1629514,1633218};

      int V_array[15] = {9685,695405,754773,756403,756673,757727,829856,854766,1323535,1323624,1323647,1323668,1323703,1521383,1629514};
      int H_array[25] ={196085,222189,357438,358531,359449,362367,384086,395817,398969,401909,402014,402015,402021,402167,755509,755553,929741,929767,929813,930466,932902,1208764,1214142,1216804,1346803};
      int passed_flag=0;
	...
	passed_flag=0;
	/*
	if(pol4_Ptr->eventNumber == 830693 || pol4_Ptr->eventNumber == 5276383 || pol4_Ptr->eventNumber == 7421818 || pol4_Ptr->eventNumber == 7460738 || pol4_Ptr->eventNumber == 8306117 || pol4_Ptr->eventNumber == 8370486 || pol4_Ptr->eventNumber == 8381355 || pol4_Ptr->eventNumber == 8394834 || pol4_Ptr->eventNumber == 8456337 || pol4_Ptr->eventNumber == 8456989 || pol4_Ptr->eventNumber == 11199059 || pol4_Ptr->eventNumber == 12233576 || pol4_Ptr->eventNumber == 15242320 || pol4_Ptr->eventNumber == 16025033 || pol4_Ptr->eventNumber == 18538216 || pol4_Ptr->eventNumber == 18543601 || pol4_Ptr->eventNumber == 18543731 || pol4_Ptr->eventNumber == 18543760 || pol4_Ptr->eventNumber == 18543785 || pol4_Ptr->eventNumber == 18543833 || pol4_Ptr->eventNumber == 22520097 || pol4_Ptr->eventNumber == 22520273 || pol4_Ptr->eventNumber == 22520379 || pol4_Ptr->eventNumber == 22520490 || pol4_Ptr->eventNumber == 22520635 || pol4_Ptr->eventNumber == 22520814 || pol4_Ptr->eventNumber == 22520861 || pol4_Ptr->eventNumber == 22520889 || pol4_Ptr->eventNumber == 22546046 || pol4_Ptr->eventNumber == 22546317 || pol4_Ptr->eventNumber == 22546425 || pol4_Ptr->eventNumber == 22548152 || pol4_Ptr->eventNumber == 22548528 || pol4_Ptr->eventNumber == 22549454 || pol4_Ptr->eventNumber == 22549948 || pol4_Ptr->eventNumber == 22550369 || pol4_Ptr->eventNumber == 22550420 || pol4_Ptr->eventNumber == 22550822 || pol4_Ptr->eventNumber == 22550859 || pol4_Ptr->eventNumber == 22550862 || pol4_Ptr->eventNumber == 22550932 || pol4_Ptr->eventNumber == 22551152 || pol4_Ptr->eventNumber == 22551210 || pol4_Ptr->eventNumber == 22551416 || pol4_Ptr->eventNumber == 22551579 || pol4_Ptr->eventNumber == 22552895 || pol4_Ptr->eventNumber == 22553352 || pol4_Ptr->eventNumber == 22553425 || pol4_Ptr->eventNumber == 22554091 || pol4_Ptr->eventNumber == 22554376 || pol4_Ptr->eventNumber == 22554487 || pol4_Ptr->eventNumber == 22674229 || pol4_Ptr->eventNumber == 22677311 || pol4_Ptr->eventNumber == 22677820 || pol4_Ptr->eventNumber == 22677829 || pol4_Ptr->eventNumber == 22677881 || pol4_Ptr->eventNumber == 23191802 || pol4_Ptr->eventNumber == 23350827 || pol4_Ptr->eventNumber == 23358924 || pol4_Ptr->eventNumber == 25887362 || pol4_Ptr->eventNumber == 26009469){
	  passed_flag=1;
	}
*/
	
	if(polarization==0){
	  if(pol4_Ptr->eventNumber == 830693 || pol4_Ptr->eventNumber == 5276383 || pol4_Ptr->eventNumber == 7421818 || pol4_Ptr->eventNumber == 7460738 || pol4_Ptr->eventNumber == 8306117 || pol4_Ptr->eventNumber == 8370486 || pol4_Ptr->eventNumber == 8381355 || pol4_Ptr->eventNumber == 8394834 || pol4_Ptr->eventNumber == 8456337 || pol4_Ptr->eventNumber == 8456989 || pol4_Ptr->eventNumber == 11199059 || pol4_Ptr->eventNumber == 12233576 || pol4_Ptr->eventNumber == 16025033 || pol4_Ptr->eventNumber == 18538216 || pol4_Ptr->eventNumber == 18543601 || pol4_Ptr->eventNumber == 18543731 || pol4_Ptr->eventNumber == 18543760 || pol4_Ptr->eventNumber == 18543785 || pol4_Ptr->eventNumber == 18543833 ||pol4_Ptr->eventNumber == 23191802 || pol4_Ptr->eventNumber == 23350827 || pol4_Ptr->eventNumber == 23358924 || pol4_Ptr->eventNumber == 25887362 || pol4_Ptr->eventNumber == 26009469){  
	  passed_flag=1;
	  cout<<"event is "<<pol4_Ptr->eventNumber<<" m is "<<m<<"\n";
	  continue;
	  }
	}
		
	if(polarization==1){
	  if(pol4_Ptr->eventNumber == 4787105 || pol4_Ptr->eventNumber == 5276383 || pol4_Ptr->eventNumber == 7382414 || pol4_Ptr->eventNumber == 7405516 || pol4_Ptr->eventNumber == 7421818 || pol4_Ptr->eventNumber == 7486629 || pol4_Ptr->eventNumber == 8099585 || pol4_Ptr->eventNumber == 8370486 || pol4_Ptr->eventNumber == 8456989 || pol4_Ptr->eventNumber == 8527491 || pol4_Ptr->eventNumber == 8529722 || pol4_Ptr->eventNumber == 8529726 || pol4_Ptr->eventNumber == 8529903 || pol4_Ptr->eventNumber == 8532837 || pol4_Ptr->eventNumber == 16024399 || pol4_Ptr->eventNumber == 16025033 || pol4_Ptr->eventNumber == 18523070 || pol4_Ptr->eventNumber == 18523574 || pol4_Ptr->eventNumber == 18524721 || pol4_Ptr->eventNumber == 18538216 || pol4_Ptr->eventNumber == 18561517 || pol4_Ptr->eventNumber == 23191802 || pol4_Ptr->eventNumber == 23350827 || pol4_Ptr->eventNumber == 23358924 || pol4_Ptr->eventNumber == 23434742 || pol4_Ptr->eventNumber == 26009469){
	...
	//passed_flag=0;
	//if(pol4_Ptr->eventNumber == 11199059) passed_flag=1;
	if(passed_flag!=1) continue;
	...
	cout<<"event,lat,lon is "<<pol4_Ptr->eventNumber<<" "<<lat<<" "<<lon<<"\n";
	...
	float limit =0.85;
	/////apply last minute cuts that were not applied before////////
	...
	lat = 90-lat;//conversion
	lon = lon+180;//conversion
	 ...
	 pixel_num=ang2pix_ring(n_side,theta,phi);//get pixel num of event (not error ellipse)
	 pixel_num_event = pixel_num;//set for later
	 ...
	 if(errorpoints_return <4) errorellipse_ctr++;//keep track of number of events that only intersect earth 2 times
	   ...
	   if(errorpoints_return >2){//use only points that intersect the earth the correct number of times. (4). If intersects 2 times there were problems down the line 
	     
	      //set Lat and Lon for error ellipse
	     
	     GetLatLonfromCartesian(Lats,Lons,anitaHeading - phiMap, anitaLat, anitaLon, xholder_temp, yholder_temp, zholder_temp);//get Lat and Lon from cartesian
	     
	     GetCartesianfromLatLon(Lats,Lons,anitaHeading-phiMap, anitaLat, anitaLon, xholder_temp, yholder_temp, zholder_temp,event_bin_height);//get coordiantes with correct height
	      
	     ToConeCoordinates(xholder,yholder,zholder, eta, xholder_temp,yholder_temp,zholder_temp, anitaAlt/1000.,0);//transform cartestion to cone coordinates
	     ...
	     for(int k=0;k<100;k++){
	       ...
	       pixel_num=ang2pix_ring(n_side,theta,phi);//get pixel num of error point
		 ...
		 if(pixel_num == pixelnum_vector[l]){//if already have that pixel number, move on to next point
		     break;
		 }
		 if(l==pixelnum_vector.size()-1){//otherwise add it to the list of bins error ellipse lies in
	     ...
	     if(errorpoints_return >2){
	       ...
	       FractionalArea_cone(error_bin,area_pix, areas,eta, anitaAlt/1000.,event_bin_height,anitaHeading - phiMap, anitaLat, anitaLon);//get fractional area
	     }
		 ...
		 if(area_pix[i]>0){
		   if(passed_flag==1){ 
		    ...
		    cout<<"event is "<<pol4_Ptr->eventNumber<<" x, y is "<<x_point<<" "<<y_point<<"\n";
		    cout<<"pixel_num is "<<pixel_num<<"\n";
		     point_sources[pointctr] = new TMarker(x_point,y_point,40);
		     //point_sources[pointctr]->SetMarkerStyle(pointctr%5+20);
		     point_sources[pointctr]->SetMarkerStyle(21);
		     point_sources[pointctr]->SetMarkerSize(1);
		     point_sources[pointctr]->SetMarkerColor(kWhite);
		     //point_sources[pointctr]->SetMarkerColor(pointctr%6+1);
		     ...
		     if(areas_frac ==1){
		       ...
		       point_boundaries[pointctr1]->SetMarkerColor(kMagenta);
		       pointctr1++;
		     }
		     
		     for(int kk=0;kk<100;kk++){
		       error_ellipse_points[errorctr] = new TMarker(xholder[kk],yholder[kk],30);
		       error_ellipse_points[errorctr]->SetMarkerStyle(pointctr%5+20);
		       error_ellipse_points[errorctr]->SetMarkerSize(0.5);
		       error_ellipse_points[errorctr]->SetMarkerColor(pointctr%6+1);
		       errorctr++;
		   ...
		   num_binned++;
		   cout<<"event_passed all cuts "<<pol4_Ptr->eventNumber<<"\n";
		   if(whole_flag ==0){
      ...
      */
      ...
      //hhealpix_map->SaveAs("90sample_healpix_formed.root");

      TH2F *haxes = new TH2F("",";x(km);y(km)",200,-500,-200,200,800,1100);
      ...
      haxes->Draw();
      TFile *sample90_map;
      TFile *sample90_map_used;
      if(polarization==0){
      sample90_map = new TFile("90sample_healpix_formed.root");
      sample90_map_used = new TFile("90sample_healpix_formed_used.root");
      }
      //if(polarization==0){
      //sample90_map = new TFile("90sample_healpix_formed_used.root");
      //}
      if(polarization==1){
      sample90_map = new TFile("90sample_healpix_formed.root");
      }
      
      TH2D *hhealpix_map = (TH2D*)sample90_map->Get("healpix_map");
      hhealpix_map->SetMinimum(1E-2);
      hhealpix_map->SetMaximum(1E5);
      ...
      TH2D *hhealpix_map_used = (TH2D*)sample90_map_used->Get("healpix_map_used");
      hhealpix_map_used->SetMinimum(1E-2);
      hhealpix_map_used->SetMaximum(1E5);
      //gStyle->SetPalette(55);
      //gStyle->SetNdivisions(10,"z");
      //gStyle=color;
      //gStyle->SetPalette(55);
      ...
      int m_val=0;
      double pval_temp=-1.;
      double fit_events_temp=-1.;
      double fit_bins_temp=-1;
	...
	fit_bins_temp=-1;
	//if(m==3030){
	if( m==3010 || m==3028 || m==3029 || m==3030 || m==3032 || m==3033 || m==3045 ||m==3046 || m==3048 || m==3051 || m==3053 || m==3057 || m==3061 || m==3062 || m==3063 || m==3066 || m==3069){
	...
	pt = new TPaveText(x_pos-100,y_pos+50,x_pos,y_pos+100);
	pt->SetFillColor(0);
	...
	for(int kk=0;kk<healpixbin_cuts_vector.size();kk++){
	  if(healpixbin_cuts_vector[kk]==m){
	    pval_temp = pval[kk];
	    fit_events_temp=fit_events[kk];
	    fit_bins_temp = fit_bins[kk];
	    cout<<"pval_temp is "<<pval_temp<<"\n";
	  }
	...
	//cout<<"x_pos, y_pos is "<<x_pos<<" "<<y_pos<<"\n";
	c3->cd(0);
	pt3 = new TPaveText(x_pos-100,y_pos-100,x_pos+100,y_pos+100);
	pt3->SetFillColor(0);
	pt3->SetFillStyle(4000);
	if(pval_temp >-1){
	  sprintf(printer,"%.2f",pval_temp);
	}
	else{
	  sprintf(printer,"No Fit!");
	}
	pt3->AddText(printer);
	pt3->Draw("same");
	...
	//////FIT EVENTS MAP
	c4->cd(0);
	pt4 = new TPaveText(x_pos-100,y_pos-100,x_pos+100,y_pos+100);
	pt4->SetFillColor(0);
	pt4->SetFillStyle(4000);
	if(fit_events_temp >-1){
	sprintf(printer,"%.2f",fit_events_temp);
	}
	else{
	  sprintf(printer,"No events");
	}
	pt4->AddText(printer);
	pt4->Draw("same");
		...
		//////FIT EVENTS MAP
	c5->cd(0);
	pt5 = new TPaveText(x_pos-100,y_pos-100,x_pos+100,y_pos+100);
	pt5->SetFillColor(0);
	pt5->SetFillStyle(4000);
	if(fit_bins_temp >-1){
	  sprintf(printer,"%.2f",fit_bins_temp);
	}
	else{
	  sprintf(printer," Bins < 4");
	}
	...
	//}
      ...
      c2->cd(0);
      for(int j=0;j<errorctr;j++){
	error_ellipse_points[j]->Draw("same");
      }
	...
	//point_boundaries[j]->Draw("same");
      ...
      if(polarization==0){
	sprintf(printer,"healpix_map_0501_90passed_%s.png","VPol");
      }
      else if(polarization==1){
	sprintf(printer,"healpix_map_0501_90passed_%s.png","HPol");
      }
      c2->Print(printer);
      //c2->Print("healpix_map_0501_90passed.eps");
      // 

       if(polarization==0){
	sprintf(printer,"healpix_map_0501_90passed_pval_%s.png","VPol");
      }
      else if(polarization==1){
	sprintf(printer,"healpix_map_0501_90passed_pval_%s.png","HPol");
      }
      c3->Print(printer);
      
       if(polarization==0){
	sprintf(printer,"healpix_map_0501_90passed_fit_%s.png","VPol");
      }
      else if(polarization==1){
	sprintf(printer,"healpix_map_0501_90passed_fit_%s.png","HPol");
      }
      c4->Print(printer);

      if(polarization==0){
	sprintf(printer,"healpix_map_0501_90passed_fitbins_%s.png","VPol");
      }
      else if(polarization==1){
	sprintf(printer,"healpix_map_0501_90passed_fitbins_%s.png","HPol");
      }
      c5->Print(printer);

       if(polarization==0){
	sprintf(printer,"healpix_map_0501_90passed_%s_used.png","VPol");
      }
      else if(polarization==1){
	sprintf(printer,"healpix_map_0501_90passed_%s_used.png","HPol");
      }
      c6->Print(printer);

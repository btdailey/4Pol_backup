...
analysis_info_4pol *pol4_Ptr_H = 0;
...
int main(int argc, char **argv) {
  ...
  TMarker *anita_loc[10000];
  TLine *anita_line[1000];
  ...
  char *rootfileH;
  rootfileH = Form(temp.c_str(),filter_name.c_str());
    
  TChain *pol4_TreeH = new TChain("analysis_info_4pol");
  TChain *extraTreeH = new TChain("extraTree");
       
 
  pol4_TreeH->Add(rootfileH);
  extraTreeH->Add(rootfileH);
    
     
  //int nevents0 = pol4_Tree->GetEntries();
  
  pol4_TreeH->SetBranchAddress("pol4_Ptr",&pol4_Ptr_H);
  ...
  //then use ang2pix  to get pix, add event to Base[pixel]
	...
	if(vflag==1){
	  ...
	  cout<<"VPOL event,lat,lon is "<<eventNumber<<" "<<lat<<" "<<lon<<"\n";
	...
	if(hflag==1){
	  polarization=1;
	  pol4_TreeH->GetEvent(m);
	  extraTreeH->GetEvent(m);
	  
	  lat = pol4_Ptr_H->sourceLat[polarization];
	  lon = pol4_Ptr_H->sourceLon[polarization];
	  eventNumber = pol4_Ptr_H->eventNumber;
	  cout<<"HPOL event,lat,lon is "<<eventNumber<<" "<<lat<<" "<<lon<<"\n";
	  
	  
	  
	  distance_from_source = pol4_Ptr_H->distance_from_source[polarization];
	  peakHilbertCoherent = pol4_Ptr_H->peakHilbertCoherent[polarization];
	  snrCoherent = pol4_Ptr_H->SNR_coherent[polarization];
	  peakVal = pol4_Ptr_H->peakVal[polarization];
	  phiMap = pol4_Ptr_H->phiMap[polarization];
	  thetaMap = pol4_Ptr_H->thetaMap[polarization];
	  ratioFirstToSecondPeak=pol4_Ptr_H->ratioFirstToSecondPeak[polarization];
	  thetaMap=pol4_Ptr_H->thetaMap[polarization];
	  
	  polFractionCoherent=pol4_Ptr_H->polFractionCoherent[polarization];

	  anitaLat = pol4_Ptr_H->anitaLat;
	  anitaLon = pol4_Ptr_H->anitaLon;
	  anitaAlt = pol4_Ptr_H->anitaAlt;
	  anitaHeading = pol4_Ptr_H->heading;
	}
	...
	lon = lon+180;//conversion
	 ...
	 if(errorpoints_return <4) errorellipse_ctr++;//keep track of number of events that only intersect earth 2 times
	   ...
	   if(errorpoints_return >2){//use only points that intersect the earth the correct number of times. (4). If intersects 2 times there were problems down the line 
	     ...
	     //if(area_pix[0] !=3028) continue;
	     cout<<"event plotting is "<<eventNumber;
	     if(errorpoints_return >2){
	       for(int i=0;i<4;i++){
		 ...
		 if(area_pix[i]>0){
		   if(passed_flag==1){ 
		    /////FOR PLOTTING EVENTS///////
		     ...
		     cout<<" x_point, y_point are "<<x_point<<" "<<y_point<<"\n";
		     ...
		     if(hflag==1){
		       point_sources[pointctr]->SetMarkerStyle(20);
		     }
		     point_sources[pointctr]->SetMarkerSize(1);
		     point_sources[pointctr]->SetMarkerColor(kWhite);

		     if(eventNumber == 25887362 || eventNumber == 8381355) point_sources[pointctr]->SetMarkerColor(kYellow);
		     //point_sources[pointctr]->SetMarkerColor(pointctr%6+1);
		     //cout<<"anita_pointx, anita_pointy are "<<anita_pointx<<" "<<anita_pointy<<"\n";
		     anita_loc[pointctr] = new TMarker(anita_pointx,anita_pointy,40);
		     anita_loc[pointctr]->SetMarkerStyle(22);
		     anita_loc[pointctr]->SetMarkerColor(kBlack);
		     anita_loc[pointctr]->SetMarkerSize(3);

		     anita_line[pointctr] = new TLine(anita_pointx, anita_pointy,x_point,y_point);
		     anita_line[pointctr]->SetLineWidth(2);
		     if(vflag==1) anita_line[pointctr]->SetLineColor(kGreen-4);
		     if(hflag==1) anita_line[pointctr]->SetLineColor(kRed-4);

		     // if(eventNumber == 18561517) anita_line[pointctr]->SetLineColor(kCyan-4);
		     ...
		     for(int kk=0;kk<100;kk++){
		       ...
		       error_ellipse_points[errorctr]->SetMarkerStyle(mm%5+20);
		       error_ellipse_points[errorctr]->SetMarkerSize(0.5);
		       error_ellipse_points[errorctr]->SetMarkerColor(mm%6+1);

		       if(eventNumber == 12233576) error_ellipse_points[errorctr]->SetMarkerColor(kTeal);
		       errorctr++;
      ...
      */
      ...
      TH2F *haxes = new TH2F("",";x(km);y(km)",200,900,1600,100,-1000,-500);
	...
	//if(m==3052){
	//pt = new TPaveText(x_pos-100,y_pos-100,x_pos,y_pos+100);
	//}
	pt->SetFillColor(0);
	...
	//point_boundaries[j]->Draw("same");
      ...
      sprintf(printer,"healpix_map_0501_90passed_all.png");

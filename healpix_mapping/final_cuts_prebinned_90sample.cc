...
int main(int argc, char **argv) {
 ...
 double xholder_temp[100]={0.};
  ...
  for(int extra=13;extra<263;extra++){
	...
	if(!fpTest){ 
	  ...
	  Pointed_Tree->Add(rootfile);
       ...
       for(int delta=0;delta<1000000;delta+=9000){
  ...
  if(polarization==0){
    temp = "HealPix_partial_0301_90sample.root";
  }
  else if (polarization==1){
    temp = "HealPix_partial_0301_90sample_HPol.root";
  }
  ...
  int healpixctr=0;
   ...
   vector<double> num_pass_in_bin(3071,0.);
   int rotatedFlag=0;
   ...
   myfile_events.open("passing_events_90sample_test.txt");
   myfile_events<<"EventNumber"<<"\t"<<"HealPix Bin"<<"\t"<<"Area in bin"<<"\n";
   ...
   //for(int m=1369290;m<nevents0;m++){
	...
	for(int j=0;j<area_pix.size();j++){
	  if(j==0) pixel_num_event = area_pix[0];

	  if(areas[j] > areas[0]) pixel_num_event = area_pix[j];
	...
	//cout<<"pixel_event_num is now "<<pixel_num_event<<"\n";
	 ...
	 inserted_flag=1;
       ...
       if(eventNumber_cut==15636066 || eventNumber_cut== 16014510 || eventNumber_cut ==14496361 || eventNumber_cut==14577785 || eventNumber_cut ==21684774 || eventNumber_cut==27146983){
	 inserted_flag=1;
       }
       //if(inserted_flag<1) continue;
	...
	y_int=100000;
	 ...
	 payloadBlastctr++;
	 if(inserted_flag==1) cout<<"inserted event "<<eventNumber_cut<<" cut by payload blast \n";
	 continue;
       }
	...
	if(inserted_flag==1){
	 ...
	 //	 if(peakVal<.03){
	 crosscorrctr++;
	 if(inserted_flag==1) cout<<"inserted event "<<eventNumber_cut<<" cut by cross corr \n";
	 continue;
       }
	 ...
	 continue;
        ...
        if(polFractionCoherent<=.3){
	 polfractionctr++;
	 if(inserted_flag==1) cout<<"inserted event "<<eventNumber_cut<<" cut by polfrac (polfrac = "<<polFractionCoherent<<"\n";
	 continue;
       }
       
	if(snrcoherent < -38*peakVal + y_int){
	  rotatedctr++;
	  if(inserted_flag==1) cout<<"inserted event "<<eventNumber_cut<<" cut by rotated. Pixel, y_int are "<<pixel_num_event<<" "<<y_int<<" snrcoherent, peakVal "<<snrcoherent<<" "<<peakVal<<"\n";
	  continue;
	}
	...
	for(int k=0;k<areas.size();k++){
	   ...
	   if(area_pix[k]>0){
		 ...
		 continue;
	       ...
	       datactr+=areas[k];
	       num_pass_in_bin[area_pix[k]]+=areas[k];
	       cout<<"event "<<eventNumber_cut<<" pixel_num is "<<area_pix[k]<<" weight is "<<areas[k]<<"\n";
	       myfile_events<<eventNumber_cut<<"\t"<<area_pix[k]<<"\t"<<areas[k]<<"\n";
	     }

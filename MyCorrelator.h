...
const int NUM_DEGREES_OFF_CENTER=75;//75;
...
class MyCorrelator 
{
 public:
  ...
  void GetNumEntries(int eventCtrStart, int eventCtrEnd, int drawMaps, int taylorFlag, int thermalFlag, std::string current_dir, int filter_number,int phase_number,int delta);

  void loopOverEvents(int eventCtrStart, int eventCtrEnd, int drawMaps, int taylorFlag, int thermalFlag, std::string current_dir, int filter_number, int phase_number,int delta);
  ...
  void GetBaselineperPhi(int pol, int nantennasToUse,std::vector<int>& whichAntennasToUse);
  //general tools
  double getMaximum(int n, double *array, int &index);
  double getRMS(TGraph *gr);
  //void GetInterpolatedGraphs();
  void GetInterpolatedGraphs(int whichPolarizations);
  ...
  double getPowerSNR(TGraph *gr);
  double getPowerSNRCoherent(TGraph *gr, int &noiseFlag);
  double getPowerSNR2(TGraph *gr);
...
void GetFFTandBaseline_old(int nantennasToUse,std::vector<int>& whichAntennasToUse,int pol, double *frequencyArray,double *baseX, double *baseY2, double *magFFT2); 
 void applyAdaptiveFilter(double centerFrequency, double bandWidth, int polFlag);
  ...
  int pointThisEvent(int eventNumber, int drawMaps,analysis_info_4pol  *pol4_Ptr);  
  ...
  int traceBackToContinent_Brian2(int eventNumber, double thetaWave, double phiWave, double &sourceLon, double &sourceLat, double &sourceAlt,double anitaLatitude, double anitaLongitude,double anitaAltitude, double anitaHeading);
  ...
  void drawCorrelationMap(double mapCorVal[NUM_BINS_ROUGH_THETA][NUM_BINS_ROUGH_PHI], int myEventNumber,int whichPolarization);
  TGraph *makeCoherentlySummedWaveform(int myEventNumber, double peakTheta, double peakPhi,
				       int nadirFlag, int polFlag, int nantennas, int drawFlag, int opp_pol);

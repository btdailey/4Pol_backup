...
analysis_info_4pol* pol4_Ptr = new analysis_info_4pol; // oindree - making pointer to struct having analysis variables that depend on all 4 pols 
...
/////////////////////////////////
void MyCorrelator::loopOverEvents(int eventCtrStart, int eventCtrEnd, int drawMaps, int taylorFlag, int thermalFlag, std::string current_dir, int filter_number,int phase_number)
{ 
 ...
 //TNtuple *ndata2= new TNtuple("ndata2","stuff to plot 2","deltaTTD:snrPeakAfterFilter:didIFilter:triggerOrPhiMaskFlag:thetaTD:phiTD:thetaWilly:phiWilly:hwTriggerAngle:thisPhiMask:distanceMcM:secondTheta:secondPhi:strongCWFlag");
  ...
  for (int eventctr=eventCtrStart;eventctr<eventCtrEnd;eventctr+=1){//LOOP START
	...
	if (isChannelSaturated(fHeadPtr->eventNumber)==-1 && !isSyncSlip(fHeadPtr->eventNumber)
	  ...
	  eventPointedFlag=pointThisEvent(fHeadPtr->eventNumber, drawMaps); 
	  tdataEvent->Fill();
	  if (printFlag==1) cout<<"Event Pointed To "<<peakThetaFinal<<", "<<peakPhiFinal
						       <<" ...about to trace back!"<<endl;
...
///////////////////////////////
double MyCorrelator::getPowerinOverlap(TGraph *gr1, TGraph *gr2, double delay){
  ...
  double min_delay = times1[0]-times2[N];
  double max_delay = times1[N]-times2[0];
  ...
  //cout<<"delay/delta_T is "<<delay/delta_t<<"\n";
  double bin_delay = delay/delta_t;//delay from angle

  double offset = (times2[0]-times1[0])/delta_t;
  //cout<<"offset is "<<times2[0]-times1[0]<<"\n";
  //cout<<"offset is "<<offset<<"\n";
  //offset=0;
  //cout<<"times1[0],times2[0] are "<<times1[0]<<" "<<times2[0]<<"\n";
  double corr_Val=0.;
  double v1_sq=0.;
  double v2_sq=0.;
  int bin1;
  ...
  for(int i=0;i<N;i++){
    ...
    //if(bin2 >N) bin2 = bin2-N;//wrapping
    //if(bin2 <0) bin2 = N+bin2;//wrapping
    //cout<<"bin1, 2 are "<<bin1<<" "<<bin2<<"\n";
    if(bin2 >=0 && (int)bin2 < N){
      // cout<<"i is "<<i<<" bin 2 is "<<bin2<<"\n";
      //cout<<"times1 2 are "<<times1[bin1]<<" "<<times2[(int)bin2]<<"\n";
      //cout<<"volts1, volts2 are "<<volts1[bin1]<<" "<<volts2[bin2]<<"\n";
  ...
  v1_sq = sqrt(v1_sq);
  v2_sq = sqrt(v2_sq);
  //cout<<"total_volts, v1,v2, corvall are "<<total_volts1<<" "<<total_volts2<<" "<<v1_sq<<" "<<v2_sq<<" "<<corr_Val<<"\n";
...
///////////////////////////////
int MyCorrelator::getPeakAntenna(int pol, int myEventNumber, int nantennas)
{
  if (!fEventTree) initialize();
  ...
  for (int ant=0;ant<nantennas;ant++){
    Int_t peakBin=FFTtools::getPeakBin(grEv[pol][ant]);
    double tVal, dummyPeakVal;
    grEv[pol][ant]->GetPoint(peakBin,tVal,dummyPeakVal);
    if (dummyPeakVal>maxPeakValAfterFilter){ 
      maxPeakValAfterFilter=dummyPeakVal;
      snrAfterFilter=getSNR(grEv[pol][ant],rmsNoiseThis);    
      antThis=ant;
    }
  }
  return antThis;
...
////////////////////////////////////
int MyCorrelator::getPeakAntenna(int myEventNumber, int nantennas,vector <int> whichAntennasToUse)
{
  if (!fEventTree) initialize();
  if (eventStartedFlag!=myEventNumber) eventStartedFlag=startEachEvent(myEventNumber);
  if (eventEntryGottenFlag!=(int)myEventNumber) eventEntryGottenFlag=getEventEntry();
  ...
  for (int i=0;i<nantennas;i++){
    ...
    //cout<<"ant is "<<ant<<" peakVal is "<<dummyPeakVal<<" SNR is "<<getSNR(grEv[ant],rmsNoiseThis)<<"\n";
    SNRs_after_filter[ant] = getSNR(grEv[ant],rmsNoiseThis);
    peakVoltage[ant] = dummyPeakVal;
    if (dummyPeakVal>maxPeakValAfterFilter){ 
      maxPeakValAfterFilter=dummyPeakVal;
      snrAfterFilter=getSNR(grEv[ant],rmsNoiseThis);
      peak2peak_signal = getPeak2Peak(grEv[ant]);
      antThis=ant;
    }
  }
  return antThis;
...
TGraph *MyCorrelator::complicatedNotchFilter(TGraph *grWave, Double_t minFreq, Double_t maxFreq, int ant, int pol,double *baseY)
{
  ...
  sprintf(filename,"/home/banerjee.104/analysis/rayleigh%s_run191.root",rayleigh_string.c_str());
						   ...
						   int nantennas, int drawFlag)
{
  ...
  if(grCoherentWaveform[polFlag] delete grCoherentWaveform[polFlag];
  ...
  */
  ...
  for(int i=0;i<40;i++){
   ...
   for(int i=0;i<40;i++){//loop over each antenna
    ...
    SNR_test = getSNR(grEv[whichPolarization][i],rmsNoise_test);
    SNR_ant[i]=SNR_test;
    ...
    times_H = grEvHoriz[i]->GetX();

    for(int n=0;n<N;n++){
      cout<<"time V,H are "<<times_V[n]<<" "<<times_H[n]<<"\n";
    }
    volts_H = grEvHoriz[i]->GetY();
    ...
    TGraph *grTemp = new TGraph(grEv[i]->GetN(),grEv[i]->GetX(),grEv[i]->GetY());
    //TGraph *grTemp = new TGraph(N,volts,grEv[i]->GetY());
    delete grEv[i];
    
    grEv[i] = new TGraph(grTemp->GetN(),grTemp->GetX(),left);
    
    TGraph *grTempH = new TGraph(grEvHoriz[i]->GetN(),grEvHoriz[i]->GetX(),grEvHoriz[i]->GetY());
    delete grEvHoriz[i];
    
    grEvHoriz[i] = new TGraph(grTempH->GetN(),grTempH->GetX(),right);
  ...
  //do correlation map
   ...
   for(int n=0;n<40;n++){
    PowerCut[n]=PowerCut[n] - integrateTDPower(grEv[whichPolarization][n]);
  }
   char namer[256];
   if(drawMaps==1){
     sprintf(namer,"5");
     DrawFreqDomain(grEv[whichPolarization][5], myEventNumber,namer);
  }
   ...
   for(int whichPolarization=0;whichPolarization<NPOL_ANALYSIS;whichPolarization++){ //oindree -- starting big whichPolarization loop here but think about it 
     ...
     if (drawMaps==1) drawCorrelationMap(mapCorVal,myEventNumber,0,whichPolarization);
     ...
     grCoherent[whichPolarization]=makeCoherentlySummedWaveform(myEventNumber, peakThetaInterp, peakPhiInterp,nadirFlag, whichPolarization, 9,drawMaps);//10
     
     // grCoherentHoriz=makeCoherentlySummedWaveform(myEventNumber, peakThetaInterp, peakPhiInterp,  nadirFlag, 1, 9,drawMaps);//10
	...
	DrawFreqDomain(grEv[whichAntennasCoherent[i]], myEventNumber,namer);
     ...
     int peakAntDummy=getPeakAntenna(whichPolarization, myEventNumber, nantennas);
     ...
     if(drawMaps==1){
       ...
       DrawFreqDomain(grCoherent[whichPolarization], myEventNumber,namer);
     }
     ...
     pol4_Ptr->snrPeakAnt[whichPolarization] = snrPeakAnt; // set really early, only based on vpol right now, but could go in getgraphsthisevent and change it
     ...
     pol4_Ptr->snrPeakAfterFilter[whichPolarization] = snrPeakAfterFilter; // 4
   ...
   polFractionCoherent=0;
   polAngleCoherent=getCoherentPolarization(grCoherent[0], grCoherentHoriz[1], polFractionCoherent);//move to after loop?
   pol4_Ptr->polFractionCoherent[0] = polFractionCoherent; // 4
   pol4_Ptr->polFractionCoherent[1] = polFractionCoherent; // 4
  //delete graphs
  delete grVertCoherentBaseline;
  delete grHorizCoherentBaseline;
				...
				int nadirFlag, int whichPolarization)
{
  ...
  if (eventStartedFlag!=myEventNumber) eventStartedFlag=startEachEvent(myEventNumber);
  if (eventEntryGottenFlag!=(int)myEventNumber) eventEntryGottenFlag=getEventEntry();
  //myCally->setClockUpSampleFactor(4); oindree - getting rid to compile, brian d thinks if data calibrated already do not need
	    ...
	    /* if ((fabs(deltaPhi1_deg*deltaPhi1_deg+
		 ...
		 for(double j=-20;j<20;j+=bin_spacing){
		   ...
		   if(bin <num_bins){
		     //cout<<"j is "<<j<<" ";
		     time_delay[bin] = j;
		     corrVal[bin] = getPowerinOverlap(grEvInterp[whichPolarization][ant1],grEvInterp[whichPolarization][ant2],j);
...
//////////////////////////////////
void MyCorrelator::drawCorrelationMap(double mapCorVal[NUM_BINS_ROUGH_THETA][NUM_BINS_ROUGH_PHI], int myEventNumber,int trial, int whichPolarization)
{
				    ...
				    int onlyTriggered, int nadirFlag, int whichPolarization)
{
  ...
  if (eventStartedFlag!=myEventNumber) eventStartedFlag=startEachEvent(myEventNumber);
  if (eventEntryGottenFlag!=(int)myEventNumber) eventEntryGottenFlag=getEventEntry();
  //myCally->setClockUpSampleFactor(4); oindree - getting rid to compile, brian d thinks do not need 
  Double_t abbyTheta=0;
  Double_t abbyPhi=0;
  int triggeredPhi[NUM_PHI_SECTORS];
  int triggeredAnt[NUM_ANTS_WITH_NADIRS];
...
TGraph *MyCorrelator::deconvolveWaveformUsingStephens(TGraph *grCoherent, int polFlag, int drawFlag)
{
 ...
 //std::ifstream impulseResponse_file(filename);
  ...
  char filename[150];
  if (polFlag==1) sprintf(filename,"/home/banerjee.104/analysis/picoSecondImpulse/antenna_hh_impulse_response.dat");
  else sprintf(filename,"/home/banerjee.104/analysis/picoSecondImpulse/antenna_impulse_response.dat");
  //cout<<filename<<endl;
...
void MyCorrelator::calcANITAIIUsingStephens()
{
  ...
  FILE *fp2;
  fp2= fopen("/home/banerjee.104/analysis/hooverCode/Aesop/data/rfcm_data/rfcm02_1.dat","r");
  ...
  FILE *fp3;
  fp3= fopen("/home/banerjee.104/analysis/hooverCode/Aesop/data/rfcm_data/rfcm01_ch2_pam01.dat","r");
...
int MyCorrelator::loadGpsTrees()
{
  ...
  //	  fCurrentRun,fCurrentRun);
  sprintf(gpsName,"/home/banerjee.104/analysis/testrun/ssDataAndToolsgpsSsFile%d.root",fCurrentRun);
  fGpsFile = TFile::Open(gpsName);
  if(!fGpsFile) {
    cout << "Couldn't open: " << gpsName << "\n";

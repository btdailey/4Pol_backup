...
mkdir -p /tmp/90sample/$RUNNUMBER/$SIMFILE/$FILTER/$PHASE/$FILENAME

FILENAMER=/tmp/90sample/$RUNNUMBER/$SIMFILE/$FILTER/$PHASE/$FILENAME

cd /home/dailey.110/analysis_oindree
source /home/dailey.110/.bash_profile

./loop90sample $RUNNUMBER $FILENAMER $FILTER $PHASE $start_number $end_number
...
then OUTPUT_DIR=geom_4pol_partial_kingbee_0301
    elif((FILTER == 4)) 

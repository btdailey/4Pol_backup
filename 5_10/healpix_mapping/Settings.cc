...
Settings::Settings() {
  Initialize();
    ...
    SKIPALLCUTS=0;//See all neutrinos, even those not seen by balloon.
...
void Settings::ReadInputs(ifstream &inputsfile, ofstream &foutput, Anita* anita1, Secondaries* sec1, Signal* sig1, Balloon* bn1, Ray* ray1) {
    ...
    if (WHICH!=2 && WHICH!=6)
	cout << "Non-default setting:  WHICH= " << WHICH << "\n";
    ...
    cout << "incline_nadir is " << anita1->INCLINE_NADIR << "\n";
    
    Tools::GetNextNumberAsString(inputsfile,foutput,number);
    
    bn1->WHICHPATH=(int)atoi(number.c_str());
    
    cout << "whichpath is " << bn1->WHICHPATH << "\n";
	...
	cout << "anita1->bwslice_required is " << anita1->bwslice_required[n] << "\n";
    }
    ...
    cout << "trigRequirements[2] is " << anita1->trigRequirements[2] << "\n";
    Tools::GetNextNumberAsString(inputsfile,foutput,number);
    ...
    cout << "zerosignal is " << ZEROSIGNAL << "\n";
    Tools::GetNextNumberAsString(inputsfile,foutput,number);
    RANDOMISEPOL=atoi(number.c_str()); 
    cout << "randomisepol is " << RANDOMISEPOL << "\n";
    Tools::GetNextNumberAsString(inputsfile,foutput,number);
    ...
    if (SKIPCUTS==1)
	cout << "Non-default setting:  Skipping all chance in hell cuts!\n";

    Tools::GetNextNumberAsString(inputsfile,foutput,number);
    SKIPALLCUTS=(int)atof(number.c_str());
    ...
    Tools::GetNextNumberAsString(inputsfile,foutput,number);
    taumodes=(int)atoi(number.c_str()); // tau modes: tau created in rock, detect tau.
    cout <<"taumodes = "<<taumodes<<endl;

...
int main() { 
   ...
   string temp = "/data/anita/btdailey/filter_study/saturated/old_phase/%s/%s/output1_0_0.root";
   int index=0;
   for(int runno=1;runno<=1;runno++){
         
     cout<<"runno is "<<runno<<"\n";
     char *rootfile = Form(temp.c_str(),filter_name.c_str(),CWevent.c_str());
       ...
       for(int extra=500;extra<31000;extra+=500){
    
	 sprintf(rootfile,"/data/anita/btdailey/filter_study/old_phase/%s/%s/output1_0_%d.root",filter_name.c_str(),CWevent.c_str(),extra);
     ...
     int eventNumber_data;
      string temp1 = "/data/anita/btdailey/icemc_data/backup/icefinal.root";
       char *rootfile1 = Form(temp1.c_str());

       TChain *IceMCFile =new TChain("passing_events");
       TChain *fEventTree = new TChain("eventTree");
       IceMCFile->Add(rootfile1);
       fEventTree->Add(rootfile1);
       for(int extra=1;extra<6;extra++){
    
	 sprintf(rootfile1,"/data/anita/btdailey/icemc_data/backup/icefinal_%d.root",extra);
	 TFile *fpTest = TFile::Open(rootfile1);
	 if(!fpTest){ 
	   cout<<"broke at extra "<<extra<<"\n";
	   continue;
	   //break;
     ...
     IceMCFile->BuildIndex("inu");
     IceMCFile->SetBranchAddress("inu",&inu);
     IceMCFile->SetBranchAddress("pnu",&pnu);
     IceMCFile->SetBranchAddress("weight",&weight);
    
     fEventTree->SetBranchAddress("UsefulAnitaEvent",&fUsefulEventPtr);
       int nevents_eventtree = fEventTree->GetEntries();
       string temp2 = "/data/anita/btdailey/filter_study/saturated/noCW/output1_0_0.root";
       char *rootfile2 = Form(temp2.c_str());
       TChain *noCWdata = new TChain("ndata");
       TChain *noCWTracing = new TChain("tdataTracing");
       noCWdata->Add(rootfile2);
       noCWTracing->Add(rootfile2);
       for(int extra=500;extra<31000;extra+=500){
    
	 sprintf(rootfile2,"/data/anita/btdailey/filter_study/saturated/noCW/output1_0_%d.root",extra);
	 TFile *fpTest = TFile::Open(rootfile2);
	 if(!fpTest){ 
	   cout<<"broke at extra "<<extra<<"\n";
	   continue;
	   //break;
	...
	//get events
       ...
       entry_number = IceMCFile->GetEntryNumberWithIndex(eventnumber2);
       IceMCFile->GetEntry(entry_number);
       fEventTree->GetEntry(entry_number);
       max_volts=-1000.;
       min_volts=1000.;
	 // cout<<"fEventTree->eventNumber is "<<fUsefulEventPtr->eventNumber<<" eventNumber2 is "<<eventnumber2<<"\n";
       for (int ant=0;ant<40;ant++){
	 
	 TGraph *gr1 = fUsefulEventPtr->getGraph(ant,AnitaPol::kVertical);
	 Double_t *volts=gr1->GetY();
	 Double_t *times=gr1->GetX();
	 
	 for(int i =0;i<gr1->GetN();i++){//gr1->GetN()
	   //cout<<"volts["<<i<<"] is "<<volts[i]<<"\n";
	   
	   if (volts[i]>max_volts){//new max Voltage
	     max_volts = volts[i];//set new abs voltage  
	   }
	   if(volts[i]<min_volts){
	     min_volts = volts[i];
	   }
       ...
       // cout<<"max_volts is "<<max_volts<<" min is "<<min_volts<<"\n";
       if(max_volts >1500) max_volts=1500;
       if(min_volts <-1500) min_volts=-1500;
       peak2peak = max_volts-min_volts;
       //cout<<"peak2peak is "<<peak2peak<<" noiseBeforeFilter is "<<noiseBeforeFilter<<"\n";
       //cout<<"icemc inu is "<<inu<<" eventnumber is "<<eventnumber2<<" eventNumber_noCW is "<<eventNumber_noCW<<"\n";
       SNRforindex = peak2peak/noiseBeforeFilter;
       // index = (int)snrPeak;
       index = (int) SNRforindex;
       //index = (int)snrPeak;
   ...
   cout<<"for CW "<<CWevent<<" and filter "<<filter_name<<"\n";
...
char printer[256];
 ...
 sprintf(printer,"/home/btdailey/analysis/filter_study/filter_efficiency/efficiency_%s_%s.png",filter_name.c_str(),CWevent.c_str());
 c1->Print(printer);
...
TStyle* RootStyle() {
  ...
  RootStyle->SetTitleOffset( 1.10,"Y");

  ...
  max_base = 1.1*max_base;
	...
	for(int m=2990;m<n_pix_int;m++){
	  ...
	  if(numberevents ==1) continue;
	    ...
	    TH1D *hdiffplot = new TH1D("diffplot",";y_intercept;Number of Events Cut;",num_bins_plots,0.,10000.);
	    TH1D *hdiffplotWhole = new TH1D("diffplotWhole",";y_intercept;Number of Events Cut;",num_bins_plots,0.,10000.);
	    TH1D *hdiffplotTotal = new TH1D("diffplotTotal",";y_intercept;Number of Events Cut;",num_bins_plots,0.,10000.);
	    ...
	    if(fit_end > fit_start +3 && numberevents >1){
	      ...
	      // if(numberevents <100){
	      hdiffplot->Fit("expo","VR WL","",fit_start,fit_end);
		//}
		//else{

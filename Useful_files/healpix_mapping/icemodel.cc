...
int IceModel::PickUnbiased(int inu,Interaction *interaction1,IceModel *antarctica,int& k, int& k1) {
    
    interaction1->PickAnyDirection(); // first pick the neutrino direction
    
    double mincos=cos(PI);//cos(COASTLINE*RADDEG);
    double maxcos=cos(0.);
    double minphi=0.;
    double maxphi=2.*PI;
    ...
    if(thiscos<=-.95 && cos(interaction1->nnu.Theta())>=.95){
      toggle=1;
      k++;
    }
	...
	int err;
	if(toggle==1 && thisnuexitearth.Lat()<=23){
	  //k1++;
	  //cout<<"IceThickness at (lon,lat) "<<thisnuexitearth.Lon()<<","<<thisnuexitearth.Lat()<<" is "<< IceThickness(thisnuexitearth)<<"\n";
	  }

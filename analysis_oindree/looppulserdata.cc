...
int main(int argc, char **argv) {
  ...
  int delta =1;
  MyCorrelator *magicPtr = new MyCorrelator("/data/anita/btdailey/pulser",run, WaveCalType::kDefault);
  cout<<"RUN NUMBER: "<<run<<endl;
  // MyCorrelator *magicPtr = new MyCorrelator("/data/anita/anitaplus/AnitaFlight0809RootData",run, WaveCalType::kDefault);
  // magicPtr->loopOverEvents(0,1000000,0,1,0,0,current_dir,filter_number,phase_number);//do taylor dome events 1000000

  magicPtr->loopOverEvents(start_event,end_event,0,1,0,current_dir,filter_number,phase_number,delta);//do taylor dome events 1000000

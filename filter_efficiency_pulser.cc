...
int main() { 
   ...
   double avgSNR_bad=0.;
   int n_bad=0;
   double avgSNR_good=0.;
   int n_good=0;

   double avgSNR_good2=0.;
   int n_good2=0;
   ...
   vector<double> weight_tracker(numbins/index_mult,0.);
    ...
    angle_cut_ctr=0;
     ...
     string temp = "/data/anita/btdailey/final_filter/pulser/geom_4pol_noisefix/output13_0.root";
   ...
   for(int runno=1;runno<=1;runno++){
	 ...
	 for(int delta=0;delta<10000;delta+=1000){
	   ...
	   sprintf(rootfile,"/data/anita/btdailey/final_filter/pulser/geom_4pol_noisefix/output%d_%d.root",extra,delta);
     ...
     cout<<"flag tree has "<<FlagTree->GetEntries()<<"\n";
	...
	//get events
       ...
       payloadBlastFlag = pol4_Ptr->payloadblastflag;
       
       if(payloadBlastFlag==1){
	 payloadBlastctr++;
	 continue;
       }
       ...
       //peakSNR_2 = pol4_Ptr->peakSNR_2[0];
       thetaTD = pol4_Ptr->deltaTheta[0];
       phiTD = pol4_Ptr->deltaPhi[0];
       varnerFlag2 = pol4_Ptr->varnerFlag2[0];
       //if(snrPeak > 14) cout<<"snrPeak is "<<snrPeak<<" event is "<<eventnumber2<<"\n";
	...
	//if(snrPeak >10 && peakVal < .1) cout<<"eventNumber is "<<eventNumber_cut<<" thetaMap is "<<thetaMap<<"\n";
	

       /* if(pol4_Ptr->peakVal[3] < pol4_Ptr->peakVal[2]){
	 continue;
       }
       */
	...
	if(eventnumber2 == 723300){
	 ...
	 hnum_peak_vol->Fill(index);
       ...
       if(varnerFlag !=1 && varnerFlag2 !=1){// && thetaMap >-35 && thetaMap <0){
	 if(peakVal >0.3 && snrPeak < 5){
	   cout<<"eventNumber for weird event is "<<eventnumber2<<"\n";
	   cout<<"SNR, ratio, peakVal, peakHilbert, elevation, triggered,polfrac are "<<snrPeak<<" "<<ratioFirstToSecondPeak<<" "<<peakVal<<" "<<peakHilbertCoherent<<" "<<thetaMap<<" "<<hwTriggerFlag<<" "<<polFractionCoherent<<" "<<pol4_Ptr->SNR_noise_bin[0]<<"\n";
	 }
	 plot_ctr++;
	 ...
	 hcross_corr_cut_corrected->Fill(peakVal,peakHilbertCoherent*distance_from_source);
	 hcross_corr_cut_SNR->Fill(peakVal,snrPeak);
	 
	 // cout<<"peakVal are "<<pol4_Ptr->peakVal[2]<<" "<<pol4_Ptr->peakVal[3]<<"\n";
	 //cout<<"box vals are "<<pol4_Ptr->peakVal_box[2]<<" "<<pol4_Ptr->peakVal_box[3]<<"\n";
	 // cout<<eventnumber2<<" was using "<<min(pol4_Ptr->peakVal[2],pol4_Ptr->peakVal[3])<<" now using "<<min(pol4_Ptr->peakVal_box[2],pol4_Ptr->peakVal_box[3])<<"\n";
	 hcross_corr_LR->Fill(min_corr,peakHilbertCoherent);
	 hcross_corr_LR_corrected->Fill(min_corr,peakHilbertCoherent*distance_from_source);
	 ...
	 limit=.85;
       ...
       /*
       if(peakHilbertCoherent >50 && peakVal <.2){
	 cout<<"bad event is "<<eventnumber2<<" SNR: "<<pol4_Ptr->snrPeakAfterFilter[0]<<"\n";
	 avgSNR_bad+=pol4_Ptr->snrPeakAfterFilter[0];
	 n_bad++;
       }
       // if(peakHilbertCoherent > 100 && peakVal >.25) cout<<"'good' right event is "<<eventnumber2<<" theta,phi is "<<thetaTD<<" "<<phiTD<<"\n";
         if(peakHilbertCoherent < 50 && peakVal <.2){
	 cout<<"'good' down event is "<<eventnumber2<<" SNR: "<<pol4_Ptr->snrPeakAfterFilter[0]<<"\n";
	  avgSNR_good+=pol4_Ptr->snrPeakAfterFilter[0];
	 n_good++;
       }

        if(peakHilbertCoherent > 50 && peakVal >.2){
	 cout<<"'good' down event is "<<eventnumber2<<" SNR: "<<pol4_Ptr->snrPeakAfterFilter[0]<<"\n";
	  avgSNR_good2+=pol4_Ptr->snrPeakAfterFilter[0];
	 n_good2++;
       }
       */
       //cout<<"good event has "<<num_bins_filtered<<" bins filtered \n";
   ...
   cout<<"events in plots is "<<plot_ctr<<"\n";
   ...
   cout<<"avgSNR_bad is "<<avgSNR_bad/n_bad<<"\n";
   cout<<"avgSNR_good is "<<avgSNR_good/n_good<<"\n";
   cout<<"avgSNR_good2 is "<<avgSNR_good2/n_good2<<"\n";
...
// TH2D  *haxes = new TH2D("axes","SNR peak After Filter; Peak-to-Peak; #frac{# passing cuts}{# passing trigger}",numbins/index_mult,0,numbins,15,0,1.1);
 ...
 sprintf(printer,"/home/dailey.110/analysis_oindree/efficiency_pulser.png");
 c1->Print(printer);
...
TCanvas *c2 = new TCanvas("c2","c2",800,800);
 ...
 sprintf(printer,"/home/dailey.110/analysis_oindree/cross_corr_dist_pulser.png");
 c2->Print(printer);

TCanvas *c3 = new TCanvas("c3","c3",800,800);
 hfilterbins->Draw();

 sprintf(printer,"/home/dailey.110/analysis_oindree/filterbins_pulser.png");
 c3->Print(printer);

 TCanvas *c5 = new TCanvas("c5","c5",800,800);
 hcross_corr_cut->Draw("zcol");

 sprintf(printer,"/home/dailey.110/analysis_oindree/cross_corr_cut_pulser.png");
 c5->Print(printer);

 TCanvas *c6 = new TCanvas("c6","c6",800,800);
 hcross_corr_cut_corrected->Draw("zcol");

 sprintf(printer,"/home/dailey.110/analysis_oindree/cross_corr_cut_corrected_pulser.png");
 c6->Print(printer);

TCanvas *c10 = new TCanvas("c10","c10",800,800);
 hdistance->Draw();

 sprintf(printer,"/home/dailey.110/analysis_oindree/distance_pulser.png");
 c10->Print(printer);
...
TCanvas *c6a = new TCanvas("c6a","c6a",800,800);
 ...
 sprintf(printer,"/home/dailey.110/analysis_oindree/cross_corr_cut_corrected_zoomed_pulser.png");
 c6a->Print(printer);
 ...
 sprintf(printer,"/home/dailey.110/analysis_oindree/cross_corr_cut_zoomed_pulser.png");
 c5a->Print(printer);

TCanvas *c7 = new TCanvas("c7","c7",800,800);
 hcross_corr_LR->Draw("zcol");

 sprintf(printer,"/home/dailey.110/analysis_oindree/cross_corr_LR_pulser.png");
 c7->Print(printer);
...
TCanvas *c7a = new TCanvas("c7a","c7a",800,800);
 ...
 sprintf(printer,"/home/dailey.110/analysis_oindree/cross_corr_LR_zoomed_pulser.png");
 c7a->Print(printer);

TCanvas *c7b = new TCanvas("c7b","c7b",800,800);
 hcross_corr_LR_corrected->Draw("zcol");

 sprintf(printer,"/home/dailey.110/analysis_oindree/cross_corr_LR_corrected_pulser.png");
 c7b->Print(printer);
...
TCanvas *c7c = new TCanvas("c7c","c7c",800,800);
 ...
 sprintf(printer,"/home/dailey.110/analysis_oindree/cross_corr_LR_zoomed_corrected_pulser.png");
 c7c->Print(printer);

TCanvas *c8 = new TCanvas("c8","c8",800,800);
 hdeltaTheta->Draw("zcol");

 sprintf(printer,"/home/dailey.110/analysis_oindree/deltaTheta_LR_pulser.png");
 c8->Print(printer);

TCanvas *c9 = new TCanvas("c9","c9",800,800);
 hdeltaPhi->Draw("zcol");

 sprintf(printer,"/home/dailey.110/analysis_oindree/deltaPhi_LR_pulser.png");
 c9->Print(printer);

TCanvas *c11 = new TCanvas("c11","c11",800,800);
 hcross_corr_cut_SNR->Draw("zcol");

 sprintf(printer,"/home/dailey.110/analysis_oindree/cross_corr_cut_SNR_pulser.png");
 c11->Print(printer);
...
sprintf(printer,"/home/dailey.110/analysis_oindree/cross_corr_cut_SNR_zoomed_pulser.png");
 c11a->Print(printer);

TCanvas *c12 = new TCanvas("c12","c12",800,800);
 hcross_corr_LR_SNR->Draw("zcol");

 sprintf(printer,"/home/dailey.110/analysis_oindree/cross_corr_LR_SNR_pulser.png");
 c12->Print(printer);
...
sprintf(printer,"/home/dailey.110/analysis_oindree/cross_corr_LR_SNR_zoomed_pulser.png");
 c12a->Print(printer);
...
TCanvas *c13 = new TCanvas("c13","c13",800,800);
 hpeakLvsR->Draw("zcol");
 line->Draw("same");
 sprintf(printer,"/home/dailey.110/analysis_oindree/peakLvsR_pulser.png");
 c13->Print(printer);

TCanvas *c14 = new TCanvas("c14","c14",800,800);
 hpeaksratio->Draw("zcol");

 sprintf(printer,"/home/dailey.110/analysis_oindree/peaksratio_pulser.png");
 c14->Print(printer);

TCanvas *c15 = new TCanvas("c15","c15",800,800);
 hcorrratio->Draw("zcol");

 sprintf(printer,"/home/dailey.110/analysis_oindree/corrratio_pulser.png");
 c15->Print(printer);

TCanvas *c16 = new TCanvas("c16","c16",800,800);
 hintegrandvsSNR->Draw("zcol");

 sprintf(printer,"/home/dailey.110/analysis_oindree/integrandSNR_pulser.png");
 c16->Print(printer);

TCanvas *c17 = new TCanvas("c17","c17",800,800);
 hdenomvsSNR->Draw("zcol");

 sprintf(printer,"/home/dailey.110/analysis_oindree/denomSNR_pulser.png");
 c17->Print(printer);

TCanvas *c18 = new TCanvas("c18","c18",800,800);
 hCorrvstime->Draw("zcol");

 sprintf(printer,"/home/dailey.110/analysis_oindree/Corrtime_pulser.png");
 c18->Print(printer);

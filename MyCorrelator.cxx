...
//analysis_info_4pol* pol4_Ptr =NULL;// new analysis_info_4pol; // oindree - making pointer to struct having analysis variables that depend on all 4 pols 
//analysis_info_once* once_Ptr = NULL; // oindree - making pointer to struct having analysis variables that dont change with pol 
int eventTree_flag=0;
...
TGraph *grEv[NPOL_ANALYSIS][NUM_ANTS_WITH_NADIRS];  //oindree - made this 2D array taking pol 
TGraph *grEvInterp[NPOL_ANALYSIS][NUM_ANTS_WITH_NADIRS];
TGraph *grCoherentWaveform[NPOL_ANALYSIS];
//oindree - check if these are needed anymore
TGraph *grEv_backup[NUM_ANTS_WITH_NADIRS];      
TGraph *grEvUnfiltered[NPOL_ANALYSIS][NUM_ANTS_WITH_NADIRS];
...
TGraph *grBaselines_avg[NUM_ANTS_WITH_NADIRS];
TGraph *grBaselines_avg2dB[NUM_ANTS_WITH_NADIRS];
/////////////////////////////////////////////////////////////////
...
vector< vector<double> > CorrNormalization;// (NUM_ANTS_WITH_NADIRS, vector<double>(0));
vector< vector<double> > CorrNormalizationBack;// (NUM_ANTS_WITH_NADIRS, vector<double>(0));
...
int passed;
double percentage;
double peakVoltage_2=0.;
double peakSNR_2=0.;

double baselineFreq[2000]={0.};
double max_correlation[NUM_ANTS_WITH_NADIRS][NUM_ANTS_WITH_NADIRS];
double RMS_ants[NUM_ANTS_WITH_NADIRS];
double max_time_delay[NUM_ANTS_WITH_NADIRS][NUM_ANTS_WITH_NADIRS];
...
double snrAfterFilter2=0.;
double noiseBeforeFilter=0.;
 double CWheight=0.;
double peak2peak_signal=0.;
...
MyCorrelator::MyCorrelator()
{
  ...
  for (int i=0;i<NUM_ANTS_WITH_NADIRS;i++){
    for (int j=0;j<NUM_ANTS_WITH_NADIRS;j++){
      for (int k=0;k<NPOL_ANALYSIS;k++){
         grEv[k][i]=0; 
	 grEvUnfiltered[k][i]=0;
         }
      grCor[i][j]=0;
    }
...
void MyCorrelator::getGraphsThisEvent(int windowWaveformFlag, double &snrPeak, double &maxSignalPeak, int &peakAnt)
{
  ...
  for (int ant=0;ant<nantennas;ant++){
    ...
    */
    //oindree - commenting out this if block to get code to compile, fitsinewave needs to be reworked because with more recent version of FFTTools cannot make instance of FFTTools
    ...
    // cout<<" ant is "<<ant<<" time0,volts0 is "<<times[0]<<" "<<volts[0]<<"\n";
    for(int i=0;i<grFiltered->GetN();i++){
      //if(ant==36) cout<<"volts[i] is "<<volts[i]<<"\n";
      volts[i]-=mean;
    ...
    if (grEv[vpol][ant]) delete grEv[vpol][ant];
    if (grEv[hpol][ant]) delete grEv[hpol][ant]; 
    if (grEv[lpol][ant]) delete grEv[lpol][ant]; 
    if (grEv[rpol][ant]) delete grEv[rpol][ant]; 

    if (grEvUnfiltered[vpol][ant]) delete grEvUnfiltered[vpol][ant];
    if (grEvUnfiltered[hpol][ant]) delete grEvUnfiltered[hpol][ant];
    if (grEvUnfiltered[lpol][ant]) delete grEvUnfiltered[lpol][ant];
    if (grEvUnfiltered[rpol][ant]) delete grEvUnfiltered[rpol][ant];
    ...
    grEv[rpol][ant]=new TGraph(grFilteredHoriz->GetN(),timesHoriz,voltsHoriz);  // change this later to be rpol 

    grEvUnfiltered[vpol][ant]=new TGraph(grFiltered->GetN(),times,volts);
    grEvUnfiltered[hpol][ant]=new TGraph(grFilteredHoriz->GetN(),timesHoriz,voltsHoriz);
    grEvUnfiltered[lpol][ant]=new TGraph(grFiltered->GetN(),times,volts);
    grEvUnfiltered[rpol][ant]=new TGraph(grFilteredHoriz->GetN(),timesHoriz,voltsHoriz);
  ...
  for (int i=0;i<ngpEvents;i++){
	...
	if (fHeadPtr->trigType&(1<<0)){ 
	  ...
	  if (isChannelSaturated(myEventNumber)==-1 && !isSyncSlip(myEventNumber)){
	    ...
	    eventPointedFlag=pointThisEvent(myEventNumber,drawMaps,pol4_Ptr); 
	    tdataEvent->Fill();
	    if (printFlag==1)cout<<"Event Pointed To "<<peakThetaFinal<<", "<<peakPhiFinal
							 <<" ...about to trace back!"<<endl;
	    
	    if (wantToTraceBackFlag==1){
	      for(int n=0;n<NPOL_ANALYSIS;n++){
		eventTracedFlag=traceBackToContinent_Brian(fHeadPtr->eventNumber, 
						 pol4_Ptr->peakThetaFinal[n],pol4_Ptr->peakPhiFinal[n], sourceLon, sourceLat, sourceAlt);

	      //pol4_Ptr->eventPointedFlag[whichPolarization] = eventPointedFlag; // 4 
		pol4_Ptr->eventTracedFlag[whichPolarization] = eventTracedFlag; // 4
		pol4_Ptr->sourceLat[whichPolarization] = sourceLat; // 4 
		pol4_Ptr->sourceLon[whichPolarization] = sourceLon; // 4
		pol4_Ptr->sourceAlt[whichPolarization] = sourceAlt; // 4
		pol4_Ptr->distance_from_source[n] = distance_from_source;
	      }
	    } 
...
/////////////////////////////////
void MyCorrelator::GetNumEntries(int eventCtrStart, int eventCtrEnd, int drawMaps, int taylorFlag, int thermalFlag, std::string current_dir, int filter_number,int phase_number,int delta)
{ 
  ...
  cout<<"eventTree_flag is "<<eventTree_flag<<"\n";
  if(eventTree_flag!=-1){
 
  cout<<"number of entries for this run is "<<fEventTree->GetEntries()<<"\n";
  }
}
/////
void MyCorrelator::loopOverEvents(int eventCtrStart, int eventCtrEnd, int drawMaps, int taylorFlag, int thermalFlag, std::string current_dir, int filter_number,int phase_number,int delta)
{ 
  cout<<"filter_num, phase num,normalization are "<<filter_number<<","<<phase_number<<" "<<normalization<<"\n";
  //  initializeBaseList();
  int wantToTraceBackFlag=1;
  if (!fEventTree) initialize();
  ...
  sprintf(filename,"%s/output%d_%d.root",current_dir.c_str(),fCurrentRun,eventCtrStart);//change this
  cout<<"outputting to file: "<<filename<<endl;
  TFile *rootfile = new TFile(filename,"RECREATE");
  
 
  TTree *pol4_tree = new TTree("analysis_info_4pol","analysis variables that depend on all 4 pols"); 
  pol4_tree->Branch("pol4_Ptr",&pol4_Ptr); 
  

// oindree - converting Ntuples below to trees with branches that are arrays to save output from all 4 pols 
/*
  double deltaTheta[NPOL_ANALYSIS];
  double deltaPhi[NPOL_ANALYSIS];
  double deltamcmTheta[NPOL_ANALYSIS]; 
  double deltamcmPhi[NPOL_ANALYSIS];
  double mapSNR[NPOL_ANALYSIS];
  double peakVal[NPOL_ANALYSIS];
  double ratioFirstToSecondPeak[NPOL_ANALYSIS];
  double snrCoherent[NPOL_ANALYSIS];
  double snrPeakAnt[NPOL_ANALYSIS];
  double maxSignalPeak[NPOL_ANALYSIS];
  double distanceTD[NPOL_ANALYSIS];
  double peakHilbertCoherent[NPOL_ANALYSIS];
  double deltaTTD[NPOL_ANALYSIS]; 
  double snrPeakAfterFilter[NPOL_ANALYSIS]; 
  int didIFilter[NPOL_ANALYSIS]; // oindree - check this type 
  double triggerOrPhiMaskFlag[NPOL_ANALYSIS];
  double thetaTD[NPOL_ANALYSIS];
  double phiTD[NPOL_ANALYSIS];
  double thetaWilly[NPOL_ANALYSIS];
  double phiWilly[NPOL_ANALYSIS];
  double hwTriggerAngle[NPOL_ANALYSIS];
  double thisPhiMask[NPOL_ANALYSIS];
  double distanceMcM[NPOL_ANALYSIS];
  double secondTheta[NPOL_ANALYSIS]; 
  double secondPhi[NPOL_ANALYSIS];
  double headingOfThisEvent[NPOL_ANALYSIS];
  int nadirFlag[NPOL_ANALYSIS];
  double thirdTheta[NPOL_ANALYSIS];
  double thirdPhi[NPOL_ANALYSIS];
  int varnerFlag[NPOL_ANALYSIS]; 
  int varnerFlag2[NPOL_ANALYSIS]; 
  double pitch[NPOL_ANALYSIS]; 
  double roll[NPOL_ANALYSIS]; 
  double heading[NPOL_ANALYSIS];  
  int phiMaskFlag[NPOL_ANALYSIS]; 
  int hwTriggerFlag[NPOL_ANALYSIS]; 
  int payloadBlastFlag[NPOL_ANALYSIS]; 
  double polAngleCoherent[NPOL_ANALYSIS]; 
  double polFractionCoherent[NPOL_ANALYSIS]; 
  int didIFilterAboveSatellite[NPOL_ANALYSIS]; 
  int didIFilterHoriz[NPOL_ANALYSIS]; //oindree - check this type 
  int didIFilterAboveSatelliteHoriz[NPOL_ANALYSIS]; 
  double meanFreqVert[NPOL_ANALYSIS]; 
  double meanFreqHoriz[NPOL_ANALYSIS]; 

  TTree *ndata = new TTree("ndata", "stuff to plot"); 
  ndata->Branch("deltaTheta",&deltaTheta,TString::Format("deltaTheta[%i]/D",NPOL_ANALYSIS));
  ndata->Branch("deltaPhi",&deltaPhi,TString::Format("deltaPhi[%i]/D",NPOL_ANALYSIS)); 
  ndata->Branch("deltamcmTheta",&deltamcmTheta,TString::Format("deltamcmTheta[%i]/D",NPOL_ANALYSIS));
  ndata->Branch("deltamcmPhi",&deltamcmPhi,TString::Format("deltamcmPhi[%i]/D",NPOL_ANALYSIS)); 
  ndata->Branch("thetaMap",&thetaMap,TString::Format("thetaMap[%i]/D",NPOL_ANALYSIS)); 
  ndata->Branch("phiMap",&phiMap,TString::Format("phiMap[%i]/D",NPOL_ANALYSIS)); 
  ndata->Branch("mapSNR",&mapSNR,TString::Format("mapSNR[%i]/D",NPOL_ANALYSIS)); 
  ndata->Branch("peakVal",&peakVal,TString::Format("peakVal[%i]/D",NPOL_ANALYSIS)); 
  ndata->Branch("ratioFirstToSecondPeak",&ratioFirstToSecondPeak,TString::Format("ratioFirstToSecondPeak[%i]/D",NPOL_ANALYSIS)); 
  ndata->Branch("snrCoherent",&snrCoherent,TString::Format("snrCoherent[%i]/D",NPOL_ANALYSIS));
  ndata->Branch("snrPeakAnt",&snrPeakAnt,TString::Format("snrPeakAnt[%i]/D",NPOL_ANALYSIS)); 
  ndata->Branch("maxSignalPeak",&maxSignalPeak,TString::Format("maxSignalPeak[%i]/D",NPOL_ANALYSIS)); 
  ndata->Branch("distanceTD",&distanceTD,TString::Format("distanceTD[%i]/D",NPOL_ANALYSIS)); 
  ndata->Branch("peakHilbertCoherent",&peakHilbertCoherent,TString::Format("peakHilbertCoherent[%i]/D",NPOL_ANALYSIS)); 
  
  TTree *ndata2 = new TTree("ndata2", "stuff to plot 2"); 
  ndata2->Branch("deltaTTD",&deltaTTD,TString::Format("deltaTTD[%i]/D",NPOL_ANALYSIS)); 
  ndata2->Branch("snrPeakAfterFilter",&snrPeakAfterFilter,TString::Format("snrPeakAfterFilter[%i]/D",NPOL_ANALYSIS)); 
  ndata2->Branch("didIFilter",&didIFilter,TString::Format("didIFilter[%i]/D",NPOL_ANALYSIS)); 
  ndata2->Branch("triggerOrPhiMaskFlag",&triggerOrPhiMaskFlag,TString::Format("triggerOrPhiMaskFlag[%i]/I",NPOL_ANALYSIS)); 
  ndata2->Branch("thetaTD",&thetaTD,TString::Format("thetaTD[%i]/D",NPOL_ANALYSIS));
  ndata2->Branch("phiTD",&phiTD,TString::Format("phiTD[%i]/D",NPOL_ANALYSIS));
  ndata2->Branch("thetaWilly",&thetaWilly,TString::Format("thetaWilly[%i]/D",NPOL_ANALYSIS)); 
  ndata2->Branch("phiWilly",&phiWilly,TString::Format("phiWilly[%i]/D",NPOL_ANALYSIS)); 
  ndata2->Branch("hwTriggerAngle",&hwTriggerAngle,TString::Format("hwTriggerAngle[%i]/D",NPOL_ANALYSIS)); 
  ndata2->Branch("thisPhiMask",&thisPhiMask,TString::Format("thisPhiMask[%i]/D",NPOL_ANALYSIS));
  ndata2->Branch("distanceMcM",&distanceMcM,TString::Format("distanceMcM[%i]/D",NPOL_ANALYSIS)); 
  ndata2->Branch("secondTheta",&secondTheta,TString::Format("secondTheta[%i]/D",NPOL_ANALYSIS)); 
  ndata2->Branch("secondPhi",&secondPhi,TString::Format("secondPhi[%i]/D",NPOL_ANALYSIS));
  ndata2->Branch("strongCWFlag",&strongCWFlag,TString::Format("strongCWFlag[%i]/I",NPOL_ANALYSIS)); 

  TTree *ndata3 = new TTree("ndata3", "stuff to plot 3"); 
  ndata3->Branch("headingOfThisEvent",&headingOfThisEvent,TString::Format("headingOfThisEvent[%i]/D",NPOL_ANALYSIS)); 
  ndata3->Branch("nadirFlag",&nadirFlag,TString::Format("nadirFlag[%i]/I",NPOL_ANALYSIS)); 
  ndata3->Branch("thirdTheta",&thirdTheta,TString::Format("thirdTheta[%i]/D",NPOL_ANALYSIS)); 
  ndata3->Branch("thirdPhi",&thirdPhi,TString::Format("thirdPhi[%i]/D",NPOL_ANALYSIS)); 
  ndata3->Branch("varnerFlag",&varnerFlag,TString::Format("varnerFlag[%i]/I",NPOL_ANALYSIS)); 
  ndata3->Branch("varnerFlag2",&varnerFlag2,TString::Format("varnerFlag2[%i]/I",NPOL_ANALYSIS)); 
  ndata3->Branch("pitch",&pitch,TString::Format("pitch[%i]/D",NPOL_ANALYSIS));
  ndata3->Branch("roll",&roll,TString::Format("roll[%i]/D",NPOL_ANALYSIS)); 
  ndata3->Branch("heading",&heading,TString::Format("heading[%i]/D",NPOL_ANALYSIS));
  ndata3->Branch("phiMaskFlag",&phiMaskFlag,TString::Format("phiMaskFlag[%i]/I",NPOL_ANALYSIS)); 
  ndata3->Branch("hwTriggerFlag",&hwTriggerFlag,TString::Format("hwTriggerFlag[%i]/I",NPOL_ANALYSIS)); 
  ndata3->Branch("ratioOfPeaksPassFlag",&ratioOfPeaksPassFlag,TString::Format("ratioOfPeaksPassFlag[%i]/I",NPOL_ANALYSIS)); 
  ndata3->Branch("elevationAnglePassFlag",&elevationAnglePassFlag,TString::Format("elevationAnglePassFlag[%i]/I",NPOL_ANALYSIS)); 
  ndata3->Branch("xCorPassFlag",&xCorPassFlag,TString::Format("xCorPassFlag[%i]/I",NPOL_ANALYSIS)); 

  TTree *ndata4 = new TTree("ndata4", "stuff to plot 4"); 
  ndata4->Branch("payloadBlastFlag",&payloadBlastFlag,TString::Format("payloadBlastFlag[%i]/I",NPOL_ANALYSIS)); 
  ndata4->Branch("polAngleCoherent",&polAngleCoherent,TString::Format("polAngleCoherent[%i]/D",NPOL_ANALYSIS)); 
  ndata4->Branch("polFractionCoherent",&polFractionCoherent,TString::Format("polFractionCoherent[%i]/D",NPOL_ANALYSIS)); 
  ndata4->Branch("didIFilterAboveSatellite",&didIFilterAboveSatellite,TString::Format("didIFilterAboveSatellite[%i]/D",NPOL_ANALYSIS)); 
  ndata4->Branch("didIFilterHoriz",&didIFilterHoriz,TString::Format("didIFilterHoriz[%i]/D",NPOL_ANALYSIS)); 
  ndata4->Branch("didIFilterAboveSatelliteHoriz",&didIFilterAboveSatelliteHoriz,TString::Format("didIFilterAboveSatelliteHoriz[%i]/D",NPOL_ANALYSIS));
  ndata4->Branch("meanFreqVert",&meanFreqVert,TString::Format("meanFreqVert[%i]/D",NPOL_ANALYSIS)); 
  ndata4->Branch("meanFreqHoriz",&meanFreqHoriz,TString::Format("meanFreqHoriz[%i]/D",NPOL_ANALYSIS)); 

*/ // getting rid of these trees that I made and replacing with struct -- oindree 
 ...
 //TNtuple *ndata2= new TNtuple("ndata2","stuff to plot 2","deltaTTD:snrPeakAfterFilter:didIFilter:triggerOrPhiMaskFlag:thetaTD:phiTD:thetaWilly:phiWilly:hwTriggerAngle:thisPhiMask:distanceMcM:secondTheta:secondPhi:strongCWFlag");
  ...
  //cout<<"LOOPING OVER EVERY 100 EVENTS!!!! \n\n\n";
   pol4_Ptr = new analysis_info_4pol;
  for (int eventctr=eventCtrStart;eventctr<eventCtrEnd;eventctr+=delta){//LOOP START
    ...
    pol4_Ptr->eventNumber = fHeadPtr->eventNumber;
    pol4_Ptr->anitaLat = anitaLatitude;
    pol4_Ptr->anitaLon = anitaLongitude;
    pol4_Ptr->anitaAlt = anitaAltitude;
    pol4_Ptr->heading = anitaHeading;
    
    pol4_Ptr->mainrfcmflag=isMainRFCMOn(fHeadPtr->eventNumber);   
    pol4_Ptr->bigenoughpeakflag=isBigEnoughPeakToPeak(fHeadPtr->eventNumber); 
    pol4_Ptr->dcoffsetflag=isDCOffsetLarge(fHeadPtr->eventNumber);
    pol4_Ptr->shorttraceflag=isShortTrace(fHeadPtr->eventNumber);
    pol4_Ptr->nadirrfcmflag=isNadirRFCMOn(fHeadPtr->eventNumber);
    pol4_Ptr->payloadblastflag=isPayloadBlast(fHeadPtr->eventNumber);
	...
	if (isChannelSaturated(fHeadPtr->eventNumber)==-1 && !isSyncSlip(fHeadPtr->eventNumber) 
	  ...
	  printf("Event: %d, the time is %.19s.%hu \n",fHeadPtr->eventNumber, timeline, 
		 timebuffer.millitm);
	  pol4_Ptr->eventNumber = fHeadPtr->eventNumber;
	  eventPointedFlag=pointThisEvent(fHeadPtr->eventNumber,drawMaps,pol4_Ptr); 
	  tdataEvent->Fill();
	  if (printFlag==1) cout<<"Event Pointed To "<<peakThetaFinal<<", "<<peakPhiFinal
						       <<" ...about to trace back!"<<endl;
	  ...
	  if (wantToTraceBackFlag==1){
		...
		eventTracedFlag=traceBackToContinent_Brian(fHeadPtr->eventNumber, 
	      ...
	      //pol4_Ptr->eventPointedFlag[whichPolarization] = eventPointedFlag; // 4 
		pol4_Ptr->eventTracedFlag[n] = eventTracedFlag; // 4
		pol4_Ptr->sourceLat[n] = sourceLat; // 4 
		pol4_Ptr->sourceLon[n] = sourceLon; // 4
		pol4_Ptr->sourceAlt[n] = sourceAlt; // 4
		pol4_Ptr->distance_from_source[n] = distance_from_source;
	      }
	...
	if (isChannelSaturated(fHeadPtr->eventNumber)==-1 && !isSyncSlip(fHeadPtr->eventNumber) 
	  ...
	  if (wantToTraceBackFlag==1){
	   for(int n=0;n<NPOL_ANALYSIS;n++){
	      ...
	      //pol4_Ptr->eventPointedFlag[n] = eventPointedFlag; // 4 
	      pol4_Ptr->eventTracedFlag[n] = eventTracedFlag; // 4
	      pol4_Ptr->sourceLat[n] = sourceLat; // 4 
	      pol4_Ptr->sourceLon[n] = sourceLon; // 4
	      pol4_Ptr->sourceAlt[n] = sourceAlt; // 4
	...
	if (isChannelSaturated(fHeadPtr->eventNumber)==-1 && !isSyncSlip(fHeadPtr->eventNumber) 
	  ...
	  eventPointedFlag=pointThisEvent(fHeadPtr->eventNumber, drawMaps,pol4_Ptr);
	  if (pol4_Ptr->peakThetaFinal[0]<0) tdataEvent->Fill();
	  if (printFlag==1) cout<<"Event Pointed To "<<peakThetaFinal<<", "<<peakPhiFinal<<endl;	  
	  if (pol4_Ptr->peakThetaFinal[0]<0){//upward pointing
	    printf("Event: %d, the time is %.19s.%hu \n",fHeadPtr->eventNumber, timeline, 
		   timebuffer.millitm);
...
//////////////////////////////////
void MyCorrelator::GetBaselineperPhi(int pol, int nantennasToUse, vector<int>& whichAntennasToUse ){
  ...
  for (int i=0;i<nantennasToUse;i++){
	...
	baseline[j]+=baselinearray[ant][j];//not in dB
      ...
      //cout<<"j is "<<j<<"baseline is "<<baseline[j]<<"\n";
    }//j
  }//i
 ...
 for(int i=0;i<NUM_ANTS_WITH_NADIRS;i++){
   ...
   /*
    for(int n=0;n<gr_holder->GetN();n++){
     if(n <gr_holder->GetN()/3) gr_holder->GetY()[n]=0.;
   ...
   */
   ...
   normalization.clear();
   normalization2.clear();
   //cout<<"corrNormalization.size() is "<<CorrNormalization.size()<<"\n";
   getCorrelationNormalizationFront(grEvInterp[whichPolarization][i],normalization);
   getCorrelationNormalizationBack(grEvInterp[whichPolarization][i],normalization2);

   CorrNormalization.push_back(normalization);
   CorrNormalizationBack.push_back(normalization2);

 }
}
///////////////////////////////
void MyCorrelator::getCorrelationNormalizationFront(TGraph *gr1,vector<double> &normalization){
  double *times = gr1->GetX();
  double *volts = gr1->GetY();
  int N = gr1->GetN();
...
///////////////////////////////
void MyCorrelator::getCorrelationNormalizationBack(TGraph *gr1, vector<double> &normalization){
  double *times = gr1->GetX();
  double *volts = gr1->GetY();
  int N = gr1->GetN();
...
double MyCorrelator::getPowerinOverlap(TGraph *gr1, TGraph *gr2, double delay, int ant1, int ant2){
  ...
  //logic: each waveform is same length.
  //for delay of t->number of bins shifted
  //lower limit = max(t0,t1+t)
  //upper limit = min(t_end0,t_end1+t);
  ...
  //cout<<"N and N2 are "<<N<<" "<<N2<<"\n";
  double delta_t = times1[1]-times1[0];
  //cout<<"delta_t is "<<delta_t<<"\n";
  double total_volts1;
  double total_volts2;
  //for(int j=0;j<N;j++){
    //total_volts1+=pow(volts1[j],2);
    //total_volts2+=pow(volts2[j],2);

  //}

  //total_volts1= sqrt(total_volts1);
  ...
  for(int i=0;i<N;i++){
    ...
    //cout<<"bin1,2 are "<<bin1<<" "<<bin2<<"\n";
    if(bin2 >=0 && (int)bin2 < N){
      //cout<<"times1 2 are "<<times1[bin1]<<" "<<times2[(int)bin2]<<"\n";
      //cout<<"volts1, volts2 are "<<volts1[bin1]<<" "<<volts2[(int)bin2]<<"\n";
      corr_Val+=volts1[bin1]*volts2[(int)bin2];
      //v1_sq+=pow(volts1[bin1],2);
      //v2_sq+=pow(volts2[(int)bin2],2);
      //num_bins++;
    }
  ...
  vector<double> normalization;
  vector<double> normalization2;
  getCorrelationNormalizationFront(gr1,normalization);
  getCorrelationNormalizationBack(gr2,normalization2);

  vector<double> normalization3;
  vector<double> normalization4;
  getCorrelationNormalizationFront(gr2,normalization4);
  getCorrelationNormalizationBack(gr1,normalization3);
  */
  //v1_sq = sqrt(v1_sq);
  //v2_sq = sqrt(v2_sq);
  
  //cout<<"v1_sq is "<<v1_sq<<" v2_sq "<<v2_sq<<"\n";

  int index_try = (int) round(total_delay); //WORKS! OUTPUT INDEX_TRY!!
  //cout<<"index_try is "<<index_try<<"\n";
  if(total_delay <=0.){
    //cout<<"normalization[(int) total delay] is "<<CorrNormalization[ant1][N+index_try-1]<<" "<<CorrNormalizationBack[ant2][N+index_try-1]<<"\n";
    v1_sq = CorrNormalization[ant1][N+index_try-1];
    v2_sq = CorrNormalizationBack[ant2][N+index_try-1];
  }
  if(total_delay >0.){
...
///////////////////////////////
///////////////////////////////
void MyCorrelator::DrawPowerinOverlap(int eventNumber, int ant1, int ant2, TGraph *gr1, TGraph *gr2, double delay){
  //new normalization for correlation. Only uses power in overlapped bins.
  ...
  int num_bins=0;
  //cout<<"delay, deltat,bin_Delay, offset is "<<delay<<" "<<delta_t<<" "<<bin_delay<<" "<<offset<<"\n";
  //cout<<"times1[0], times1[N] is "<<
  TH2D *hintegrand = new TH2D("integrand",";time;Value",N,times1[0],times1[N-1],1E5,-500,1800);
  TH2D *hdenominator = new TH2D("hdenominator",";time;Value",N,times1[0],times1[N-1],1E5,0,1800);
  ...
  for(int i=0;i<N;i++){
    ...
    if(bin2 >=0 && (int)bin2 < N){
      ...
      corr_Val+=volts1[bin1]*volts2[(int)bin2];
      rms_ant1 = getRMSOfRange(gr1,0,i+1);
      rms_ant2 = getRMSOfRange(gr2,0,i+1);
      wtf = rms_ant1*rms_ant2;
      //cout<<"ant1, ant2 is "<<rms_ant1<<" "<<rms_ant2<<" wtf is "<<wtf<<"\n";
      //cout<<"rms1 2 are "<<rms_ant1<<" "<<rms_ant2<<"\n";
      hintegrand->Fill(times1[i],corr_Val/1021);
      hdenominator->Fill(times1[i],wtf);
     
    }

  }//i
  char printer[256];
  TCanvas *cIntegrand = new TCanvas("cIntegrand","cIntegrand",800,800);
  
  hintegrand->Draw();

  sprintf(printer,"integrand_%i_%i_%i.png",eventNumber, ant1,ant2);
  cIntegrand->Print(printer);

  TCanvas *cDenom = new TCanvas("cDenom","cDenom",800,800);
  hdenominator->Draw();

  sprintf(printer,"denominator_%i_%i_%i.png",eventNumber, ant1,ant2);
  cDenom->Print(printer);
...
/////////////////////////////////////////
int MyCorrelator::getPeakAntenna(int myEventNumber, int nantennas,vector <int> whichAntennasToUse, int whichPolarization)
{
if (!fEventTree) initialize();
  ...
  for (int i=0;i<nantennas;i++){
    ant = whichAntennasToUse[i];
    //cout<<"ant is "<<ant<<"\n";
    Int_t peakBin=FFTtools::getPeakBin(grEv[whichPolarization][ant]);
    double tVal, dummyPeakVal;
    peakVoltage_2=0.;
    peakSNR_2=0.;
    grEv[whichPolarization][ant]->GetPoint(peakBin,tVal,dummyPeakVal);
    //cout<<"ant is "<<ant<<" peakVal is "<<dummyPeakVal<<" SNR is "<<getPowerSNR(grEv[whichPolarization][ant])<<" rms is "<<RMS_ants[ant]<<"\n";
    //cout<<"old SNR would be "<<getSNR(grEv[whichPolarization][ant],rmsNoiseThis)<<"\n";
    if (dummyPeakVal>maxPeakValAfterFilter){ 
      peakVoltage_2 = maxPeakValAfterFilter;
      peakSNR_2 = snrAfterFilter;


      maxPeakValAfterFilter=dummyPeakVal;
      if(normalization==0){
	snrAfterFilter=getSNR(grEv[whichPolarization][ant],rmsNoiseThis);
      }
      else{
	snrAfterFilter=getPowerSNR(grEv[whichPolarization][ant]);
	snrAfterFilter2 =getPowerSNR2(grEv[whichPolarization][ant]);
      }
      peak2peak_signal = getPeak2Peak(grEv[whichPolarization][ant]);
      antThis=ant;
      //cout<<"ant is "<<ant<<" snr is "<<SNRs_after_filter[ant]<<" power_SNR is "<<getPowerSNR(grEv[whichPolarization][ant])<<"\n";
      //cout<<"maxpeakVal is now "<<maxPeakValAfterFilter;
      //cout<<" snrPeak is now "<<snrAfterFilter<<"\n";
    }
    
    else if(dummyPeakVal > peakVoltage_2){
      peakVoltage_2 = dummyPeakVal;
      peakSNR_2 = getPowerSNR(grEv[whichPolarization][ant]);
    }
  }
  return antThis;
}
/////////////////////////////////////////
double MyCorrelator::getPowerSNR(TGraph *gr){
  double max_power=-1.;
  double power=0.;
  ...
  int n_5 = numpoints/5;
  /*for(int i=0;i<n_5;i++){
    noise_power+=pow(y[i],2);

  }
  */
  //cout<<"noise_power = "<<noise_power<<"\n";
  for(int i=10;i<4*n_5;i++){//exclude first 1/5 of waveform
    power=0.;
    for(int j =0;j<n_5;j++){
      power+=pow(y[i+j],2);
    }//j
  ...
  end_noise_bin = start_bin;
  if(start_bin > n_5) end_noise_bin = n_5; 

  for(int i=0;i<end_noise_bin;i++){
    noise_power+=pow(y[i],2);
...
////////////////////////////////////
/////////////////////////////////////////
double MyCorrelator::getPowerSNRCoherent(TGraph *gr, int &noiseFlag){
  double max_power=-1.;
  double power=0.;
  double noise_power=0.;
  int start_bin;
...
/////////////////////////////////////////
double MyCorrelator::getPowerSNR2(TGraph *gr){
  double max_power=-1.;
  double power=0.;
  double noise_power=0.;
...
int MyCorrelator::isChannelSaturated(int myEventNumber){
  ...
  for(int i = 0;i<nantennas*2;i++){//fill out array for L and R pols
    if(saturatedChannels[i]==1 || saturatedChannels[nantennas+i]==1){
      saturatedChannels[2*nantennas+i]=1;
      saturatedChannels[3*nantennas+i]=1;
    }
...
void MyCorrelator::adaptiveFilter(int pol, double dBCut, int nfreq, double *frequencies, int drawFlag, double bandWidth, float &mean_freq, double *freqArray, double *FFTarray, double *baseX, double *baseY, int baseN, int myEventNumber)// double &mean)
{
  ...
  for (int ant=0;ant<NUM_ANTS_WITH_NADIRS;ant++){      
    if ((pol!=0 || ant!=1) && saturatedChannels[ant+pol*NUM_ANTS_WITH_NADIRS]==0){//get rid of 2V
      ...
      Y = grEv[pol][ant]->GetY(); //set X and Y values 
      X = grEv[pol][ant]->GetX(); 
 
      deltaT=X[1]-X[0];
      
      length=grEv[pol][ant]->GetN();
						...
						int myEventNumber, int nadirFlag, int antenna_groups)
{
  ...
  GetBaselineperPhi(pol,nantennasToUse, whichAntennasToUse);//take n antennas and get the avg baseline for those
...
void MyCorrelator::applyAdaptiveFilter(double centerFrequency, double bandWidth, int polFlag)
{
  ...
  double baseY[100];
    ...
    if (centerFrequency!=-1){
      for (int ant=0;ant<nantennas;ant++){
	TGraph *gr1 = new TGraph(grEv[polFlag][ant]->GetN(),grEv[polFlag][ant]->GetX(),grEv[polFlag][ant]->GetY());
	delete grEv[polFlag][ant];
	...
	grEv[polFlag][ant]=new TGraph(grNotch->GetN(),grNotch->GetX(),grNotch->GetY());
	delete gr1;
	delete grNotch;
      }//ant
    }//centerfreq
...
void MyCorrelator::applySatelliteFilter(double centerFrequency, double bandWidth,int ant,int pol,double *baseY)
{
  ...
  if (centerFrequency!=-1){
	...
	//cout<<"ant being filtered by satellite is "<<ant<<"\n";
	  ...
	  TGraph *gr1 = new TGraph(grEv[pol][ant]->GetN(),grEv[pol][ant]->GetX(),grEv[pol][ant]->GetY());
	  delete grEv[pol][ant];
	 
	  if(notchFilterFlag==0){
	    TGraph *grNotch = nofillNotchFilter(gr1,centerFrequency-bandWidth,centerFrequency+bandWidth);
	    grEv[pol][ant]=new TGraph(grNotch->GetN(),grNotch->GetX(),grNotch->GetY());
	    delete gr1;
	    delete grNotch;
	  }
	  if(notchFilterFlag==1){
							      ...
							      int nantennas, int drawFlag)
{
  ...
  for (int i=0;i<nantennas;i++){
    ...
    //cout<<"ant: "<<ant<<endl;
    if (polFlag==0) grEvCoherent[i]=deconvolveWaveformUsingStephens(grEvUnfiltered[0][ant],0,0);//grEvUnfiltered
  }
...
TGraph *MyCorrelator::makeCoherentlySummedWaveform(int myEventNumber, double peakTheta, 
						   double peakPhi, int nadirFlag, int polFlag,
						   int nantennas, int drawFlag, int opp_pol)
{
  if (eventStartedFlag!=myEventNumber) eventStartedFlag=startEachEvent(myEventNumber);
  if (eventEntryGottenFlag!=(int)myEventNumber) eventEntryGottenFlag=getEventEntry();
  TGraph *grEvCoherent[nantennas];
  ...
  if(printFlag==1) cout<<"peakPhi is "<<peakPhi<<"\n";
  getClosestNAntennas(nantennas, peakPhi, whichAntennasToUse,nadirFlag);
  if(opp_pol==0){//getting coherent waveform for opposite pol for pol frac!
    for(int k=0;k<nantennas;k++){
      if(k<nantennas){
	whichAntennasCoherent.push_back(whichAntennasToUse[k]);
	//cout<<"("<<polFlag<<") Antennas to use are "<<whichAntennasToUse[k]<<"\n";
  ...
  for (int i=0;i<nantennas;i++){
    int ant=whichAntennasToUse[i];
    //sprintf(namer,"%i",ant);
    //DrawFreqDomain(grEv[0][ant],myEventNumber,namer);
    //cout<<"using ant "<<ant<<"\n";
    grEvCoherent[i]=FFTtools::getInterpolatedGraph(grEv[polFlag][ant], deltaTInt); //vertical pol grEvUnfiltered
  ...
  double max_timeExpected=0.;
  int ant=0;
  double timeExpected=0.;
  for (int i=0;i<nantennas;i++){
    ant=whichAntennasToUse[i];
    //cout<<"antenna "<<whichAntennasToUse[i]<<" used for coherently summed waveform"<<endl;
    timeExpected=getDeltaTExpected(ant, whichAntennasToUse[0], 
							  peakPhi, peakTheta);

    if (abs(timeExpected) > max_timeExpected) max_timeExpected = abs(timeExpected);
  }
  ...
  for (int i=0;i<nantennas;i++){
	...
	if(voltVal == voltVal){
	  ...
	  //cout<<"j is "<<j<<" bin1,2 "<<bin1<<" "<<bin2<<" tVal1, tval2 "<<tVal1<<" "<<tVal2<<" val1,val2, "<<val1<<" "<<val2<<" volt is "<<voltVal<<"\n";
     
	}
 }
    }
    
    /* for (int j=0;j<npoints-factorToLeaveOut*upSampleFactor;j++){//start upsample and iterpolate
      int bin1=int((timeExpected+j*deltaTInt-tVal0)/lengthOfTrace*grEvCoherent[i]->GetN());
      int bin2=bin1+1;
      //if (j==factorToLeaveOut*upSampleFactor) cout<<"bin1: "<<bin1<<", bin2: "<<bin2<<endl;
      if (bin1>=0 && bin2<=grEvCoherent[i]->GetN()){
  ...
  //for plotting, get rid of 0's
  //cout<<"polFlag is "<<polFlag<<"\n";
  double timesCoherentPlot[npoints], voltsCoherentPlot[npoints];
   for (int j=0;j<npoints-2*factorToLeaveOut*upSampleFactor;j++){
    
    timesCoherentPlot[j]=timesCoherent[j+factorToLeaveOut*upSampleFactor];
    voltsCoherentPlot[j]=voltsCoherent[j+factorToLeaveOut*upSampleFactor];
    //cout<<"volts is "<<voltsCoherentPlot[j]<<"\n";
  }
  
   /* for (int j=0;j<npoints-factorToLeaveOut*upSampleFactor;j++){
    timesCoherent[j]=deltaTInt*j;
    voltsCoherent[j]=voltsCoherent[j]/nantennas;
    //myfile<<timesCoherent[j]<<"\t"<<voltsCoherent[j]<<"\n";
    }
  for (int j=0;j<npoints-2*factorToLeaveOut*upSampleFactor;j++){
    ...
    timesCoherentPlot[j]=timesCoherent[j];
    voltsCoherentPlot[j]=voltsCoherent[j];
    //cout<<"volts is "<<voltsCoherentPlot[j]<<"\n";
  }
   */
  if(grCoherentWaveform[polFlag]) {
    
    delete grCoherentWaveform[polFlag];
  }
  
  grCoherentWaveform[polFlag]=new TGraph(npoints-(2*upSampleFactor*factorToLeaveOut)-1,
				      timesCoherentPlot,voltsCoherentPlot);
    
  if (drawFlag==1 && opp_pol==0){//draw coherent waveform
    char printer[256];
   
      TCanvas *cCoherent=new TCanvas("cCoherent","cCoherent",800,300);
      cCoherent->cd(0);
      grCoherentWaveform[polFlag]->GetXaxis()->SetTitle("Time (ns)");
      grCoherentWaveform[polFlag]->GetYaxis()->SetTitle("Voltage (mV)");
      grCoherentWaveform[polFlag]->Draw("al");
      grCoherentWaveform[polFlag]->SetMinimum(-60);
  ...
  */
  return grCoherentWaveform[polFlag];
}
///////////////////////////////////////
///////////////////////////////////////
int MyCorrelator::pointThisEvent(int eventNumber, int drawMaps, analysis_info_4pol* pol4_Ptr) 
...
{ 
  ...
  cout<<" payloadBlast = "<<isPayloadBlast(fHeadPtr->eventNumber)<<"\n";
  cout<<" saturated channels is "<<isChannelSaturated(fHeadPtr->eventNumber)<<"\n";
  cout<<" mainrfcmflag= "<<isMainRFCMOn(fHeadPtr->eventNumber)<<"\n";
  cout<<"bigenoughpeakflag= "<<isBigEnoughPeakToPeak(fHeadPtr->eventNumber)<<"\n";
  cout<<"dcoffsetflag= "<<isDCOffsetLarge(fHeadPtr->eventNumber)<<"\n";
  cout<<"shorttraceflag= "<<isShortTrace(fHeadPtr->eventNumber)<<"\n";
  cout<<"nadirrfcmflag= "<<isNadirRFCMOn(fHeadPtr->eventNumber)<<"\n";
  
   pol4_Ptr->eventNumber = fHeadPtr->eventNumber;
    pol4_Ptr->anitaLat = fAdu5APatPtr->latitude;
    pol4_Ptr->anitaLon = fAdu5APatPtr->longitude;
    pol4_Ptr->anitaAlt = fAdu5APatPtr->altitude;
    pol4_Ptr->heading = fAdu5APatPtr->heading;
  ...
  double deltaTheta=0;  //oindree trying to make array work
  double deltaPhi=0;
  double thetaMap=0;
  double phiMap=0;
  double phi=0;
  ...
  double thetaWilly=0;
  double phiWilly=0;
  double peakPhiFinal=0;
  double peakThetaFinal=0;
  int phiMaskArray[NUM_PHI_SECTORS];
  ...
  */
  ...
  double mapCorValRefined2[NUM_BINS_FINE_THETA][NUM_BINS_FINE_PHI];
  ...
  TGraph *grCoherent[NPOL_ANALYSIS];
  ...
  */
  ...
  for(int antenna_groups=0;antenna_groups<antenna_groups_size;antenna_groups++){//go through all phi sectors
	...
	for(int j1=0;j1<50;j1++){
	  ...
	  if(antennaFreq[antused][j1]<=0 && n<nfreq){ 
	    //cout<<"cutting frequencies : "<<frequencies[n]<<"\n";
  ...
  /*
     char namer[256];
  sprintf(namer,"34_before");
  DrawFreqDomain(grEv[0][34], myEventNumber,namer);
  */
  for(int i=0;i<40;i++){
   ...
   for(int i=0;i<40;i++){//loop over each antenna
    if(i!=1 && saturatedChannels[i]==0 && saturatedChannels[i+NUM_ANTS_WITH_NADIRS]==0 ){//dont use bad antennas
	...
	PeakMag[i][j] = PeakMag_backup[i][j];
      ...
      GetBaselineperPhi(0,1, whichAntennasToUse_1);
      GetBaselineperPhi(1,1, whichAntennasToUse_1);
      //
	...
	for(int k=0;k<(int)uniquefreqs1.size();k++){
	  bins_holder0 = 2*uniquebandwidth[k];
	  num_bins_filtered += bins_holder0/deltaF_bin;
	  cout<<"cutting V "<<uniquefreqs1[k]-uniquebandwidth[k]<<" to "<<uniquefreqs1[k]+uniquebandwidth[k]<<"\n";
	...
	if(uniquefreqs1[k]>0) applyAdaptiveFilter_singleAnt(uniquefreqs1[k], bandWidth,0,i);//apply filter in Vert
	
      }//k
      
	//	cout<<"num_bins_filterd is "<<num_bins_filtered<<"\n";
	for(int k=0;k<(int)uniquefreqsHoriz1.size();k++){
	  cout<<"cutting H "<<uniquefreqsHoriz1[k]-uniquebandwidthHoriz[k]<<" to "<<uniquefreqsHoriz1[k]+uniquebandwidthHoriz[k]<<"\n";
	...
	if(uniquefreqsHoriz1[k]>0) applyAdaptiveFilter_singleAnt(uniquefreqsHoriz1[k],bandWidth,1,i);//apply filter in Horiz
    ...
    const int nfour = 512;
    double left[nfour/2];
    double right[nfour/2];
    double total[nfour/2];
    double timestep;
    double *times_V;
    double *volts_V;
    double *times_H;
    double *volts_H;
    int N = grEv[vpol][i]->GetN();
    times_V = grEv[vpol][i]->GetX();
    volts_V = grEv[vpol][i]->GetY();

    times_H = grEv[hpol][i]->GetX();

  
    volts_H = grEv[hpol][i]->GetY();
    timestep = times_V[1]-times_V[0];
    double delayV_H = .065;//ns
    //cout<<"timestep = "<<timestep<<"\n";
    //cout<<"bin delay is "<<int(delayV_H/timestep)<<"\n";
    //ShiftRight(volts_V,nfour/2,int(delayV_H/timestep));
    ConvertHVtoLRTimedomain(nfour,volts_V,volts_H,left,right);
    
    for(int j=0;j<nfour/2;j++){
      total[j] = left[j]+right[j];
      // cout<<"total is "<<total[j]<<" = "<<left[j]<<" "<<right[j]<<"\n";
    }

    TGraph *grTemp = new TGraph(grEv[vpol][i]->GetN(),grEv[vpol][i]->GetX(),grEv[vpol][i]->GetY());
    //TGraph *grTemp = new TGraph(N,volts,grEv[i]->GetY());
    delete grEv[lpol][i];
    
    grEv[lpol][i] = new TGraph(grTemp->GetN(),grTemp->GetX(),left);
    
    TGraph *grTempH = new TGraph(grEv[hpol][i]->GetN(),grEv[hpol][i]->GetX(),grEv[hpol][i]->GetY());
    delete grEv[rpol][i];
    
    grEv[rpol][i] = new TGraph(grTempH->GetN(),grTempH->GetX(),right);
   ...
   }//END OF FILTERING-NEED BOTH V AND H
  ...
  //do correlation map
   ...
   antennaFreq.clear();
   antennaBandwidth.clear();
   PeakMag.clear();
   antennaFreqHoriz.clear();
   antennaBandwidthHoriz.clear();
   PeakMagHoriz.clear();
   ...
   vector <vector <double> >().swap(antennaFreq);
   vector <vector <double> >().swap(antennaBandwidth);
   vector <vector <double> >().swap(PeakMag);
   vector <vector <double> >().swap(antennaFreqHoriz);
   vector <vector <double> >().swap(antennaBandwidthHoriz);
   vector <vector <double> >().swap(PeakMagHoriz);

   vector<double>().swap(uniquefreqs1);
   vector <double>().swap(uniquefreqsHoriz1);
   vector <double>().swap(uniquebandwidth);
   vector <double>().swap(uniquebandwidthHoriz);
   vector <double>().swap(uniquePhase);
   vector <double>().swap(uniquePhaseHoriz);
   vector <double>().swap(uniquePhase_bandwidth);
   ...
   /////////Start loop over whichPol?
    int theta_bin_V=0;
   int phi_bin_V=0;
   double theta_final_V=0.;
   double phi_final_V=0.;
   double peakVal_box=0.;
   int theta_bin=0;
   int phi_bin=0;
    for(int whichPolarization=0;whichPolarization<4;whichPolarization++){ //oindree -- starting big whichPolarization loop here but think about it 
...
s     }
	...
	peakVal_box=peakVal;
	theta_final_V = peakTheta;
	phi_final_V = peakPhi;
     }
     
     else{

       cout<<"peakTheta,Phi are "<<theta_final_V<<" "<<phi_final_V<<"\n";
       findPeakOfMapinBox(mapCorVal,peakVal_box,theta_final_V,phi_final_V);
     ...
     cout<<"peakVal is "<<peakVal<<" peakVal in box is "<<peakVal_box<<"\n";
     ...
     //make grCoherent into [npol_analysis]!!!
   ...
   if(drawMaps==1){
      ...
      cTime_used->Divide(3,3);
      int used_ctr=1;
      TCanvas *cAmplitude = new TCanvas("cAmplitude","cAmplitude",4000,1200);
      cAmplitude->Divide(16,4);
      //TH2F *haxes_amp = new TH2F

      TCanvas *cTime_used_unfiltered = new TCanvas("cTime_used_unfiltered","cTime_used_unfiltered",800,880);
      cTime_used_unfiltered->Divide(3,3);

      TCanvas *cAmplitude_used_unfiltered = new TCanvas("cAmp_used_unfiltered","cAmp_used_unfiltered",800,880);
      cAmplitude_used_unfiltered->Divide(3,3);

      TCanvas *cAmplitude_used = new TCanvas("cAmp_used","cAmp_used",800,880);
      cAmplitude_used->Divide(3,3);
	 ...
	 used_flag=0;
	 if(i==1) i++;
	 for(int k=0;k<whichAntennasCoherent.size();k++){
	  
	   if(i==whichAntennasCoherent[k]) used_flag=1;
	 }
	 ...
	 TGraph *gfft = f.makePowerSpectrumMilliVoltsNanoSecondsdB(grEv[pol_flag][i]);
	 gfft->GetXaxis()->SetTitle("Freq");
	 gfft->GetYaxis()->SetTitle("Power (dB)");
	 gfft->SetMinimum(-20);
	 gfft->SetMaximum(40);

	 TGraph *gfft2 = f2.makePowerSpectrumMilliVoltsNanoSecondsdB(grEvUnfiltered[pol_flag][i]);
	 gfft2->GetXaxis()->SetTitle("Freq");
	 gfft2->GetYaxis()->SetTitle("Power (dB)");
	 gfft2->SetMinimum(-20);
	 gfft2->SetMaximum(40);
	 ...
	 //j=i+1;
	 if(i <8) j=2*i+1;
	 else if (i<16) j = 2*i+2;
	 else if (i<32) j=i+17;
	 else if(i>31) j=2*i-15;
	 
	 //cout<<"i is "<<i<<" j is "<<j<<"\n";
	 cTime->cd(j);
	 TPad *pad1 = (TPad *)(cTime->cd(j));
	 pad1->SetLeftMargin(0);
	  pad1->SetRightMargin(0);
	 pad1->Draw();
	 if(used_flag==1){
	   gPad->SetFrameBorderMode(3); 
	   gPad->SetFrameLineColor(kGreen);
	   gPad->SetFrameLineWidth(6);    
	 }
	 grEv[pol_flag][i]->Draw("al");

	 if(used_flag==1){
	 cTime_used->cd(used_ctr);
	 grEv[pol_flag][i]->Draw("al");
	

	 cTime_used_unfiltered->cd(used_ctr);
	 grEvUnfiltered[pol_flag][i]->Draw("al");
	 
	 cAmplitude_used_unfiltered->cd(used_ctr);
	 gfft2->Draw("al");

	 cAmplitude_used->cd(used_ctr);
	 gfft->Draw("al");
	 ...
	 cAmplitude->cd(j);
	 TPad *pad2 = (TPad *) (cAmplitude->cd(j));
	...
	grCoherent[0] = makeCoherentlySummedWaveform(myEventNumber, peakThetaInterp, peakPhiInterp,  nadirFlag, 0, 9,drawMaps,1);//10
	polAngleCoherent=getCoherentPolarization(grCoherent[0], grCoherent[1], polFractionCoherent);
      }
   ...
   //HACK!
     ...
     int peakAntDummy=getPeakAntenna(myEventNumber, whichAntennasCoherent.size(),whichAntennasCoherent,whichPolarization);
     //peakAntDummy=0;
     
     sigmaIndex=int(snrAfterFilter);
     cout<<"SNR_coherent vs SNR is "<<SNR_coherent<<" "<<snrAfterFilter<<"\n";
     //cout<<"snrAfterfilter is "<<snrAfterFilter<<"\n";
     cout<<"peakAnt is "<<peakAntDummy<<"\n";
     if (sigmaIndex>14) sigmaIndex=14;
     
     int ant1;
     int ant2;
     for(int k=0;k<whichAntennasCoherent.size();k++){
       for(int k1 =k+1;k1<whichAntennasCoherent.size();k1++){
	 ant1 = min(whichAntennasCoherent[k],whichAntennasCoherent[k1]);
	 ant2 = max(whichAntennasCoherent[k],whichAntennasCoherent[k1]);
	 //cout<<" max_correlation["<<ant1<<"]["<<ant2<<"] is "<<max_time_delay[ant1][ant2]<<"\n";
       }
	 ...
	 sprintf(namer,"ant%i",whichAntennasCoherent[k]);
	 DrawFreqDomain(grEv[0][whichAntennasCoherent[k]], myEventNumber,namer);
       }
     ...
     */
     ...
     if(drawMaps==1 && whichPolarization==0){
       ...
       DrawFreqDomain(grCoherent[0], myEventNumber,namer);
     }
     */
     snrCoherent=getSNR(grCoherent[whichPolarization],rmsNoiseCoherent);
     SNR_ant_coherent=0.;
     SNR_ant_coherent=snrCoherent;
     
     peakHilbertCoherent=0;
     peakHilbertCoherent=getPeakHilbert(grCoherent[whichPolarization]);
     if (printFlag==1) cout<<"peak hilbert coherent: "<<peakHilbertCoherent<<endl;
     cout<<"peakHilbertCoherent is "<<peakHilbertCoherent<<" peakVal is "<<peakVal<<"\n";
     ...
     cout<<"Ratio of 1st to 2nd Peak in Map: "<<ratioFirstToSecondPeak<<endl;
     ...
     int varnerflag2=isVarnerEvent2(fHeadPtr->eventNumber);
     int varnerflag=isVarnerEvent(fHeadPtr->eventNumber);
     ...
     ////////////////////////Fill outputs
     pol4_Ptr->SNR_noise_bin[whichPolarization] = end_noise_bin;
     pol4_Ptr->deltaTheta[whichPolarization] = deltaTheta; //changes with map, needs 4 pols 
     pol4_Ptr->deltaPhi[whichPolarization] = deltaPhi; // 4
     pol4_Ptr->deltamcmTheta[whichPolarization] = deltamcmTheta; // 4
     pol4_Ptr->deltamcmPhi[whichPolarization] = deltamcmPhi; // 4
     pol4_Ptr->mapSNR[whichPolarization] = mapSNR; // 4
     pol4_Ptr->peakVal[whichPolarization] = peakVal; // 4
     pol4_Ptr->ratioFirstToSecondPeak[whichPolarization] = ratioFirstToSecondPeak; //4
     pol4_Ptr->snrCoherent[whichPolarization] = snrCoherent; // 4, changes with antennas you use which is based on map
     pol4_Ptr->snrPeakAnt[whichPolarization] = peakAntDummy;//snrPeakAnt; // set really early, only based on vpol right now, but could go in getgraphsthisevent and change it
     pol4_Ptr->maxSignalPeak[whichPolarization] = maxSignalPeak; //only based on vpol right now, probably need for all 4 
     
     pol4_Ptr->peakHilbertCoherent[whichPolarization] = peakHilbertCoherent; // 4
     pol4_Ptr->polFractionCoherent[whichPolarization] = polFractionCoherent; // 4
  
   
     pol4_Ptr->snrPeakAfterFilter[whichPolarization] = snrAfterFilter; // 4
     pol4_Ptr->snrPeakAfterFilter2[whichPolarization] = snrAfterFilter2;
     pol4_Ptr->didIFilter[whichPolarization] = didIFilter; // 4
     pol4_Ptr->triggerOrPhiMaskFlag[whichPolarization] = triggerOrPhiMaskFlag; // 4
     pol4_Ptr->thetaMap[whichPolarization] = thetaMap;
     pol4_Ptr->phiMap[whichPolarization] = phiMap;
     
     pol4_Ptr->secondThetaMap[whichPolarization]= secondThetaMap; // 4 
     pol4_Ptr->secondPhiMap[whichPolarization] = secondPhiMap; // 4 
     pol4_Ptr->headingOfThisEvent[whichPolarization] = headingOfThisEvent; // 4, outputs heading - peakphi of map, so changes with map 
     
     pol4_Ptr->tertiaryThetaMap[whichPolarization] = tertiaryPeakTheta; // 4,  CHANGED NAME SO ALSO CHANGE NAME IN POINTTHISEVENT
     pol4_Ptr->tertiaryPhiMap[whichPolarization] = tertiaryPeakPhi; // 4,    CHANGED NAME SO ALSO CHANGE NAME IN POINTTHISEVENT 
     pol4_Ptr->varnerFlag[whichPolarization] = varnerflag;  // 4, SEE IF CAN THROW OUT    
     pol4_Ptr->varnerFlag2[whichPolarization] = varnerflag2; // 4
     
     pol4_Ptr->phiMaskFlag[whichPolarization] = phiMaskFlag; // 4, dependent on peak phi 
     pol4_Ptr->hwTriggerFlag[whichPolarization] = hwTriggerFlag; // 4
     
     pol4_Ptr->polAngleCoherent[whichPolarization] = polAngleCoherent; // 4 
     
     pol4_Ptr->didIFilterAboveSatellite[whichPolarization] = didIFilterAboveSatellite; // 4
     //pol4_Ptr->didIFilterHoriz[whichPolarization]; // do not need 
     //pol4_Ptr->didIFilterAboveSatelliteHoriz[whichPolarization]; // do not need this anymore 
     
     // pol4_Ptr->meanFreqVert[whichPolarization] = meanFreqVert; // 4, do we need all 4? 
     //pol4_Ptr->meanFreqHoriz[whichPolarization]; // do not need this anymore 
     
     
     pol4_Ptr->peakThetaFinal[whichPolarization] = peakThetaFinal;
     pol4_Ptr->peakPhiFinal[whichPolarization] = peakPhiFinal;
     //pol4_Ptr->finaltheta; // not needed 
     /* if(drawMaps==1){
       pol4_Ptr->eventPointedFlag[whichPolarization] = eventPointedFlag; // 4 
       pol4_Ptr->eventTracedFlag[whichPolarization] = eventTracedFlag; // 4
       pol4_Ptr->sourceLat[whichPolarization] = sourceLat; // 4 
     ...
     */
     pol4_Ptr->rmsNoiseCoherent[whichPolarization] = rmsNoiseCoherent; // 4 
     pol4_Ptr->noiseBeforeFilter[whichPolarization] = noiseBeforeFilter; // 4  //we need to code this more
     pol4_Ptr->CWheight[whichPolarization] = CWheight; // 4
     //pol4_Ptr->SNR_ant[whichPolarization] = SNR_ant; // 4, add in index 
     //pol4_Ptr->SNR_ant_triggered[whichPolarization]; // 4, add in index 
     //pol4_Ptr->SNR_ant_closest[whichPolarization]; // 4, add in index 
     //pol4_Ptr->SNR_ant_coherent[whichPolarization]; // 4, add in index 
     //pol4_Ptr->PowerCut[whichPolarization]; // 4, add in index, might not keep as array of 40 antennas
     //pol4_Ptr->CoherentAnts[whichPolarization]; // 4, list of antennas 
     //pol4_Ptr->distance_from_source[whichPolarization] = distance_from_source; // 4 
     pol4_Ptr->peak2peak_signal[whichPolarization] = peak2peak_signal; // 4, depend on after filtering which will change for each pol 

     pol4_Ptr->SNR_coherent[whichPolarization] = SNR_coherent;
     pol4_Ptr->SNR_coherent2[whichPolarization] = SNR_coherent2;
     pol4_Ptr->noiseFlag[whichPolarization] = badnoiseFlag;


     pol4_Ptr->peakVoltage_2[whichPolarization] = peakVoltage_2;
     pol4_Ptr->peakSNR_2[whichPolarization] = peakSNR_2;

     pol4_Ptr->peakVal_box[whichPolarization] = peakVal_box;
     pol4_Ptr->num_bins_filtered = num_bins_filtered;

     pol4_Ptr->realTime = fHeadPtr->realTime;
     for(int i=0;i<NUM_ANTS_WITH_NADIRS;i++){
       pol4_Ptr->RMS_ants[whichPolarization][i]=RMS_ants[i];
       for(int j=i;j<NUM_ANTS_WITH_NADIRS;j++){
	 pol4_Ptr->max_correlation[whichPolarization][i][j] = max_correlation[i][j];
	 
	 
	 pol4_Ptr->time_delay[whichPolarization][i][j]=max_time_delay[i][j];
       }
     }
   ...
   }// oindree -- big loop over whichPolarization ends here  
  ...
  grHorizCoherentBaseline=0;
  whichAntennasCoherent.clear();
  for(int whichPolarization=0;whichPolarization<NPOL_ANALYSIS;whichPolarization++){ //oindree -- putting last whichPolarization loop here but think about it 
    //delete grCoherent[whichPolarization];
    //delete grCoherent2[whichPolarization];
  ...
  for (int ant1=0;ant1<NUM_ANTS_WITH_NADIRS;ant1++){
    ...
    delete grEv[whichPolarization][ant1];
    delete grEvInterp[whichPolarization][ant1];
    delete grEv_backup[ant1];
    delete grEvUnfiltered[whichPolarization][ant1];
    ...
    //delete fAdu5APatPtr;
    //delete fUsefulAdu5Ptr;

    grEv[whichPolarization][ant1]=0;
    grEvInterp[whichPolarization][ant1]=0;
    grEv_backup[ant1]=0;
    grEvUnfiltered[whichPolarization][ant1]=0;
    grEvHorizUnfiltered[ant1]=0;
    grVertBaseline[ant1]=0;
    grHorizBaseline[ant1]=0;

  }
} //last whichPolarization ends here 
...
void MyCorrelator::drawRefinedMap(double mapCorValRefined[NUM_BINS_FINE_THETA][NUM_BINS_FINE_PHI], 
				  double peakTheta, double peakPhi,int myEventNumber,int polarization) 
{
				...
				int nadirFlag, int whichPolarization)
{
 if (eventStartedFlag!=myEventNumber) eventStartedFlag=startEachEvent(myEventNumber);
  ...
  //for(int ant=0;ant<nantennas;ant++) cout<<"rms["<<ant<<"] is "<<rms[ant]<<"\n";
  ...
  int flagged=0;
  int num_bins=416;
  double bin_spacing = 40./num_bins;
  double time_delay[num_bins];
  double corrVal[num_bins];
  int bin=0;
  TGraph *gr_holder1;
  TGraph *gr_holder2;
  ...
  for (int ant1=0;ant1<nantennas;ant1++){
	...
	if (allowedFlag==1 && ((peakPhi>lowerAngleThis && peakPhi<higherAngleThis) 
	  ...
	  if(cos_reconstruction_flag==1){
		...
		//cout<<"ctrPhi, peakPhi, phiStartOffset is "<<ctrPhi<<" "<<peakPhi<<" "<<phiStartOffset<<"\n";
		//cout<<"NUM_BINS_FIN_PHI, FINE_BIN_SIZE is "<<NUM_BINS_FINE_PHI<<" "<<FINE_BIN_SIZE<<"\n";
		...
		for (int ctrTheta=0;ctrTheta<NUM_BINS_FINE_THETA;ctrTheta++){
		  ...
		  //cout<<"ctrTheta, ctrPhi is "<<ctrTheta<<" "<<ctrPhi<<" corVal is "<<corVal<<"\n";
		  //cout<<"ant 1 is "<<ant1<<" 2 is "<<ant2<<" phi,theta, corval is "<<abbyPhi_deg<<" "<<abbyTheta_deg<<" "<<corVal<<"\n";
		  //cout<<"rms are "<<rms[ant1]<<" "<<rms[ant2]<<"\n";
		  if(rms[ant1] <1E-10 || rms[ant2]<1E-10){
		    ...
		    //cout<<"corVal/rms is "<<corVal/(rms[ant1]*rms[ant2])<<"\n";
  ...
  //cout<<"peak of refined map is "<<peakRefinedVal<<"\n";
  for (int i=0;i<NUM_BINS_FINE_THETA;i++){
    for (int j=0;j<NUM_BINS_FINE_PHI;j++){
  ...
  for ( int ant1 = 0; ant1 < nantennas; ant1++ ) {
	...
	if ( allowedFlag == 1 ) {
	  ...
	  if( draw_flag == 1 ){
	    ...
	    c_volts->Divide(1,3);
	    c_volts->cd(1);
	    grEv[whichPolarization][ant1]->Draw();
	    c_volts->cd(2);
	    grEv[whichPolarization][ant2]->Draw();
	    c_volts->cd(3);
	    haxes_corr->Draw();
	    ...
	    c_volts1->Divide(1,2);
	    c_volts1->cd(1);
	    grEvInterp[whichPolarization][ant1]->Draw("AL");
	    c_volts1->cd(2);
	    grShifted->Draw("AsL");
...
///////////////////////////
void MyCorrelator::findPeakOfMapinBox(double mapCorVal[NUM_BINS_ROUGH_THETA][NUM_BINS_ROUGH_PHI], double &peakVal, double peakTheta, double peakPhi)
{
  peakVal=0;
  double min_phi = peakPhi-10;
  double max_phi = peakPhi+10;
  //cout<<"acos(peakTheta) is "<<acos(peakTheta)<<" "<<10*deg2rad<<"\n";
  //cout<<"min_Phi, max_Phi are "<<min_phi<<" "<<max_phi<<"\n";
   
   double peakTheta_val=0.;
   peakTheta_val = acos(peakTheta);
   
   peakTheta_val = pi/2 - peakTheta_val;
   
   peakTheta_val = peakTheta_val*rad2deg;
   
   
   //cout<<"peakTheta_Val is "<<peakTheta_val<<"\n";
   double max_theta = peakTheta_val +10;
   double min_theta = peakTheta_val -10;

   max_theta = max_theta *deg2rad;
   max_theta = pi/2 - max_theta;
   max_theta = cos(max_theta);

   min_theta = min_theta *deg2rad;
   min_theta = pi/2 - min_theta;
   min_theta = cos(min_theta);
   //cout<<"min_theta and max_theta are "<<min_theta<<" "<<max_theta<<"\n";
  int i_min = (int)((min_theta - cosHighestTheta )/step_size_cos_rough);
  int i_max = (int)((max_theta - cosHighestTheta)/step_size_cos_rough);
  double val_bin = (int)((peakTheta - cosHighestTheta )/step_size_cos_rough);
  int j_min = (int)((min_phi/ROUGH_BIN_SIZE));
  int j_max = (int)((max_phi/ROUGH_BIN_SIZE));
 
  int j_map=0;
  // cout<<"Val_bin in theta is "<<val_bin<<"\n";
  //cout<<"i_min, i_max are "<<i_min<<" "<<i_max<<"\n";
  //cout<<"j_min, j_max are "<<j_min<<" "<<j_max<<"\n";

  //cout<<"values for theta are min, val, max: "<<min_theta<<" "<<peakTheta<<" "<<max_theta<<"\n";

  if(i_min < 0) i_min=0;
  if(j_min < 0) j_min=0;

  if(i_max > NUM_BINS_ROUGH_THETA) i_max = NUM_BINS_ROUGH_THETA;
  for (int i=i_min;i<i_max;i++){
    for (int j=j_min;j<j_max;j++){
      // cout<<"theta and phi is "<<i*step_size_cos_rough + cosHighestTheta<<" "<<j*ROUGH_BIN_SIZE<<"\n";
      j_map = j;
      //cout<<"i,j are "<<i<<" "<<j<<" mapCorVal is "<<mapCorVal[i][j_map]<<"\n";
      if(j> NUM_BINS_ROUGH_PHI) j_map -=NUM_BINS_ROUGH_PHI;
      if (mapCorVal[i][j_map]>peakVal){
	peakVal=mapCorVal[i][j_map];
	//cout<<"i,j(j_map) are "<<i<<" "<<j<<"("<<j_map<<") "<<peakVal<<"\n";
...
{
  if (!fAntarctica) initializeAntarctica();
  cout<<"past antarctica, need anita position? \n";
...
//////////////////////////////////
/////////////////////////////////////
int MyCorrelator::traceBackToContinent_Brian2(int eventNumber, double thetaWave, double phiWave, double &sourceLon, double &sourceLat, double &sourceAlt,double anitaLatitude,double anitaLongitude,double anitaAltitude,double anitaHeading)
{
  if (!fAntarctica) initializeAntarctica();
  if (!fUPGeomTool) fUPGeomTool=AnitaGeomTool::Instance();
   if (!fRampdemReader)fRampdemReader =RampdemReader::Instance();
   ...
   */
   ...
   //cout<<"getting roll and pitch axis \n";
   TVector3 rollAxis,pitchAxis;
   rollAxis=fUPGeomTool->fRollRotationAxis;
   pitchAxis=fUPGeomTool->fPitchRotationAxis;
   ...
   TVector3 rollAxis2,pitchAxis2,headingAxis2;
   //cout<<"about to do some transforms \n";
   //cout<<"rollAxis is "<<rollAxis[0]<<" "<<rollAxis[1]<<" "<<rollAxis[2]<<"\n";
   fUPGeomTool->getCartesianCoords(anitaLatitude,anitaLongitude,anitaAltitude,Anita_Cart);
   AnitaPos.SetXYZ(Anita_Cart[1],Anita_Cart[0],Anita_Cart[2]);
...
TGraph *MyCorrelator::deconvolveWaveformUsingStephens(TGraph *grCoherent, int polFlag, int drawFlag)
{
  ...
  FILE *fp1;
  if (polFlag==0) fp1=fopen("/home/banerjee.104/analysis/systemImpulseResponse/systemImpulseResponseUsingStephens.txt","r");
  ...
  //else fp1 = fopen("/home/dailey.110/abby/agoodhue/anitaIICode/myCorrelator/systemImpulseResponse/systemImpulseResponseNoAntenna.txt","r");
    ...
    fp1= fopen("/home/banerjee.104/analysis/anitaIICode/myCorrelator/systemImpulseResponse/ch1hAbby.tpl","r");
...
int MyCorrelator::getEventEntry()
{
  ...
  if(fUseCalibratedEventFile) {
    ...
    cout<<"Warning: you are trying to use uncalibrated data and we don't know how to deal with that yet because we don't have kVTFullAGCrossCorClock in latest version of eventReaderRoot";


    //oindree - commenting next 4 lines to get code  to compile
    //if(fUsefulEventPtr)
    //delete fUsefulEventPtr;
...
{       
  ...
  if(1) {
    ...
    if (!fGotCalEventFile && !fGotEventFile){
      ...
      simdata_flag=1;
      sprintf(eventName,"%s/SimulatedAnitaEventFile.root",fCurrentBaseDir);
      sprintf(headerName,"%s/SimulatedAnitaHeadFile.root",fCurrentBaseDir);
      cout<<"eventName is "<<eventName<<"\n";
      fEventTree = new TChain("eventTree");
      fEventTree->Add(eventName);
      ...
      // fEventTree = new TChain("eventTree");
      // fEventTree->Add(eventName);
      for(int extra=1;extra<10;extra++) {
	sprintf(eventName,"%s/SimulatedAnitaEventFile%d.root",fCurrentBaseDir,extra);
	TFile *fpTest = TFile::Open(eventName);
	if(!fpTest){
    ...
    // if(fCurrentRun ==1){
      ...
      cout<<"headername is "<<headerName<<"\n";
      fHeadTree = new TChain("headTree");
      fHeadTree->Add(headerName);

      for(int extra=1;extra<10;extra++) {
	sprintf(headerName,"%s/SimulatedAnitaHeadFile%d.root",fCurrentBaseDir,extra);
	TFile *fpTest = TFile::Open(headerName);
	if(!fpTest){
	   cout<<"broke at extra = "<<extra<<"\n";
	  break;
	}
...
int MyCorrelator::loadGpsTrees()
{
 if (fAdu5APatPtr) delete fAdu5APatPtr;
  ...
  //	  fCurrentRun,fCurrentRun);
  sprintf(gpsName,"/home/dailey.110/analysis_oindree/testrun/ssDataAndToolsgpsSsFile%d.root",fCurrentRun);
  fGpsFile = TFile::Open(gpsName);
  fAdu5aPatTree = new TChain("adu5PatTree");
  if(!fGpsFile) {
    cout << "Couldn't open: " << gpsName << "\n";

    if(fCurrentRun==1){
      sprintf(gpsName,"%s/SimulatedAnitaGpsFile.root",fCurrentBaseDir);
  ...
  if(fCurrentRun==1){
    for(int extra=1;extra<10;extra++) {
      sprintf(gpsName,"%s/SimulatedAnitaGpsFile%d.root",fCurrentBaseDir,extra);
      TFile *fpTest = TFile::Open(gpsName);
      if(!fpTest){
	cout<<"broke at extra = "<<extra<<"\n";
	break;
      }
...
// oindree - commenting out fitsinewave function because it needs to be reworked as recent version of FFTTools does not allow instance of FFTTools
/*
...
void MyCorrelator::GeomMethod(int ant,int pol,vector<double> Freq,vector<double> bandWidth,vector<double> cutFreqs){
  ...
  times = grEv[pol][ant]->GetX();
  volts = grEv[pol][ant]->GetY();
  ...
  for(int i=0;i<newLength;i++) {
    ...
    delete grEv[pol][ant];
    grEv[pol][ant] = new TGraph(length,times1,filteredVals);
...
void MyCorrelator::DrawFreqDomain(TGraph *gr, int eventnumber,char *namer){
  ...
  TCanvas *c0 = new TCanvas("c0","c0",800,800);
    ...
    c0->cd(2);
    haxes_mag->Draw();
    grBaselines_avg[phi_sector]->Draw("same l");
    //grBaselines_avg2dB[phi_sector]->Draw("l");
    //grBaselines_avg2dB[phi_sector]->SetLineColor(kRed);
    grMag->Draw("same");
  ...
  for(int k=0;k<length;k++){//grEv[ant]->GetN()
    ...
    */
	...
	volts[i+time_offset]=filteredVals[i];
      ...
      TH2F *htime_axes = new TH2F("time_axes",";Time(ns);Volts(mV)",10,0,100,10,-30,40);
     TGraph *grtime_shifted = new TGraph(grEv[pol][ant]->GetN(),grEv[pol][ant]->GetX(),grEv[pol][ant]->GetY());
...
//}//InterpPhase
void MyCorrelator::ConvertHVtoLRTimedomain(const int nfour,double *vvolts,double *hvolts,double *left,double *right) {
...
void MyCorrelator::realft(double *data, const int isign, int nsize){
    int i, i1, i2, i3, i4;
...
void MyCorrelator::four1(double *data, const int isign,int nsize) {
...
void MyCorrelator::SWAP(double &a, double &b){
    double dum = a;
    a = b;
    b = dum;
}
void MyCorrelator::ShiftRight(double *x,const int n,int ishift) {
